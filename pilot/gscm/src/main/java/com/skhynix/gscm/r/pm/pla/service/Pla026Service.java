...
@Service
public class Pla026Service {

    @Autowired
    private Pla026Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 getRecordSet("AUTH_LIST")의 반환 타입/구조에 프레임워크 의존
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA026QrySelectMainList(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<String, Object>();
	try {
		...
		// FIXME(원본 버그): 원본은 du.dPLA02601(requestData, onlineCtx).getRecordSet("MAIN_LIST") 구조이나, 프레임워크 제거로 동일 객체/메서드 체인을 그대로 유지할 수 없음
		Object rs = store.dPLA02601(request).get("MAIN_LIST");
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA026QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 onlineCtx 미선언 상태로 사용함
		Object rs = store.dPLA02602(request, onlineCtx).get("DETAIL_LIST");
