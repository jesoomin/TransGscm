...
@Service
public class Pla041Service {

    @Autowired
    private Pla041Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 getRecordSet("AUTH_LIST")의 결과를 IRecordSet으로 받아 getRecordCount()를 호출한다. Spring 포팅에서는 NEXCORE 의존 제거로 인해 동일 타입을 사용할 수 없어 원본 동작을 그대로 표현하기 어렵다.
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA041QrySelectMainList(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 du.dPLA04101(requestData, onlineCtx).getRecordSet("MAIN_LIST") 호출 구조였으나,
		// NEXCORE 의존 제거 후에도 반환 타입/구조가 불명확함. 원본 로직 그대로 유지 차원에서 get("MAIN_LIST") 형태로만 이식.
		Object rs = ((Map<String, Object>) store.dPLA04101(request)).get("MAIN_LIST");
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA041QrySelectDetail(Map<String, Object> request) {
	Map<String, Object> responseData = new HashMap<>();
	try {
		Map<String, Object> paramMap = request;
		...
		// FIXME(원본 버그): 원본의 onlineCtx 미선언 상태를 그대로 유지하여 포팅 불가
		Map<String, Object> result = store.dPLA04102(request);
		responseData.put("DETAIL_LIST", result.get("DETAIL_LIST"));
	} catch (BizRuntimeException be) {
		throw be;
	} catch (Exception e) {
		throw new BizRuntimeException("E0052", e);
