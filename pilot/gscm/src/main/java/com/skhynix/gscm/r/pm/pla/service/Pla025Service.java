...
@Service
public class Pla025Service {

    @Autowired
    private Pla025Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 getRecordSet("AUTH_LIST") / IRecordSet / getRecordCount()에 의존하므로, 프레임워크 제거 후에도 동일 로직을 그대로 유지하기 어렵다.
		Object rs = duResult.get("AUTH_LIST");
		// FIXME(원본 버그): 원본의 IRecordSet rs.getRecordCount() 호출을 그대로 옮길 수 없어 컴파일 가능한 범위에서만 치환했다.
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA025QrySelectMainList(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 호출은 NEXCORE IDataSet 전용 동작이며 Map 기반 포팅에서는 request 자체를 수정하는 것으로만 유지됨.

		// FIXME(원본 버그): 원본의 onlineCtx 미사용/미이관. 프레임워크 의존 제거 요구에 따라 전달하지 않음.
		Map<String, Object> storeResult = store.dPLA02501(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA025QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 requestData.putFieldMap(paramMap)를 호출하지만 Spring Map 요청 객체에서는 동일 개념이 없음. 원본 로직 유지 차원에서 paramMap=request 자체에 반영된 상태로 둠.

		Map<String, Object> storeResponse = store.dPLA02502(request);
