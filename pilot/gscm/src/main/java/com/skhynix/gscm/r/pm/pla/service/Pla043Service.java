...
@Service
public class Pla043Service {

    @Autowired
    private Pla043Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
    ...
    try {
        ...
        // FIXME(원본 버그): 원본은 IRecordSet rs 및 rs.getRecordCount()에 프레임워크 타입 의존이 있으나 그대로 포팅해야 하므로 컴파일 불가 가능성 있음
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA043QrySelectMainList(Map<String, Object> request) {
	...
	try {
		...
		Map<String, Object> storeResult = store.dPLA04301(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA043QrySelectDetail(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<>();
	try {
		Map<String, Object> paramMap = request;
		...
		// FIXME(원본 버그): 원본은 requestData/onlineCtx/responseData(IDataSet)/IRecordSet 등 NEXCORE 타입 전제를 그대로 사용함
		Object rs = store.dPLA04302(request).get("DETAIL_LIST");
