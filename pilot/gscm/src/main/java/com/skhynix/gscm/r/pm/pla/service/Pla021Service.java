...
@Service
public class Pla021Service {

    @Autowired
    private Pla021Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		// FIXME(원본 버그): 원본은 DPLA021 du = (DPLA021) lookupDataUnit(DPLA021.class); 형태의 NEXCORE lookupDataUnit 의존을 사용함
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA021QrySelectMainList(Map<String, Object> request) {
    ...
    try {
        ...
        // FIXME(원본 버그): 원본은 du.dPLA02101(requestData, onlineCtx).getRecordSet("MAIN_LIST")를 호출하나, Spring 포팅 시 requestData/onlineCtx/getRecordSet 개념이 없음. 원본 로직을 그대로 유지하기 위해 결과를 MAIN_LIST로 매핑만 수행.
        Object rs = store.dPLA02101(request).get("MAIN_LIST");
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA021QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 호출이 있었으나 Spring Map 기반 포팅에서는 동일 객체(paramMap=request)에 직접 반영됨

		Object rs = store.dPLA02102(request).get("DETAIL_LIST");
