...
@Service
public class Pla010Service {

    @Autowired
    private Pla010Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA010QrySelectMainList(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<>();
	try {
		// FIXME(원본 버그): 원본은 requestData를 사용하지만 선언되어 있지 않음. 원본 그대로 유지할 수 없어 request를 대체 사용.
		Map<String, Object> paramMap = request;

		// FIXME(원본 버그): 원본은 requestData.getField(...)를 사용하지만 requestData가 미선언 상태임.
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap)은 requestData 미선언으로 그대로 이식 불가. request에 반영된 상태로 대체.

		// FIXME(원본 버그): 원본은 onlineCtx를 사용하지만 선언되어 있지 않음.
		Map<String, Object> result = store.dPLA01001(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA010QrySelectDetail(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<>();
	try {
		Map<String, Object> paramMap = request;
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 동작을 그대로 유지할 대상 객체(requestData)가 Spring Map 전환 후 사라짐

		Map<String, Object> result = store.dPLA01002(request);
