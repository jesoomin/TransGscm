...
@Service
public class Pla013Service {

    @Autowired
    private Pla013Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
    ...
    try {
        ...
        // FIXME(원본 버그): 원본은 IRecordSet rs로 선언하고 rs.getRecordCount()를 호출하나, 프레임워크 의존 제거 후 구체 타입 정보가 없어 그대로 컴파일되지 않을 수 있음.
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA013QrySelectMainList(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<>();
	try{
		...
		Map<String, Object> storeResult = store.dPLA01301(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA013QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본은 du.dPLA01302(requestData, onlineCtx).getRecordSet("DETAIL_LIST") 형태로 NEXCORE IDataSet/IOnlineContext 및 RecordSet 반환을 전제함
		Object rs = store.dPLA01302(request).get("DETAIL_LIST");
