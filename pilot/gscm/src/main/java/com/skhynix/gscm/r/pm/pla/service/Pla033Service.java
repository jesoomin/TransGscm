...
@Service
public class Pla033Service {

    @Autowired
    private Pla033Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		// FIXME(원본 버그): 원본은 requestData, onlineCtx를 사용하지만 선언/전달되지 않음
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA033QrySelectMainList(Map<String, Object> request) {
    ...
    try {
        ...
        // FIXME(원본 버그): 원본은 requestData, onlineCtx, getRecordSet("MAIN_LIST") 사용에 프레임워크 의존이 있으며 그대로 포팅 시 타입/호출이 불명확함
        Object rs = store.dPLA03301(request).get("MAIN_LIST");
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA033QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 동작을 그대로 대응할 NEXCORE 객체가 제거되어 별도 반영 없이 동일 Map에 put 처리만 유지함.

		Object rs = store.dPLA03302(request).get("DETAIL_LIST");
