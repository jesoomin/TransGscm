...
@Service
public class Pla024Service {

    @Autowired
    private Pla024Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		Object _result = store.dAuthCheck(request);
		// FIXME(원본 버그): 원본은 du.dAuthCheck(requestData, onlineCtx).getRecordSet("AUTH_LIST")를 호출하나, requestData/onlineCtx는 미선언이며 반환타입/구조도 명확하지 않다.
		java.util.List rs = (java.util.List) ((java.util.Map<String, Object>) _result).get("AUTH_LIST");
		if (rs.size() > 0) {
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA024QrySelectMainList(Map<String, Object> request) {
	...
	try {
		...
		Map<String, Object> storeResult = store.dPLA02401(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA024QrySelectDetail(Map<String, Object> request) {
	...
	try {
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 동작을 그대로 옮길 프레임워크 객체가 없어 request 맵 갱신으로만 대체됨.

		Map<String, Object> storeResult = store.dPLA02402(request);
