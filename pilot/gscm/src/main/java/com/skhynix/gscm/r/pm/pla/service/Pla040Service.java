...
@Service
public class Pla040Service {

    @Autowired
    private Pla040Store store;
...
public Map<String, Object> fAuthCheck(Map<String, Object> request) {
	...
	try {
		// FIXME(원본 버그): 원본은 requestData, onlineCtx 미선언 상태로 사용함. 원본 그대로 유지 차원에서 request를 requestData에 대응시켜 포팅.
		Map<String, Object> requestData = request;
		Map<String, Object> duResult = store.dAuthCheck(requestData);
		// FIXME(원본 버그): 원본은 getRecordSet("AUTH_LIST")의 반환 타입/구조에 강하게 의존함. 프레임워크 제거 포팅으로 동일 의미만 유지.
		Object authListObj = duResult.get("AUTH_LIST");
		int recordCount = 0;
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA040QrySelectMainList(Map<String, Object> request) {
    ...
    try {
        ...
        // FIXME(원본 버그): requestData.putFieldMap(paramMap); 원본의 requestData가 이식 대상 코드에 미선언 상태였으나 원본 로직 그대로 유지 불가

        // FIXME(원본 버그): onlineCtx 미선언
        Map<String, Object> storeResult = store.dPLA04001(request);
    ...
    // LLM 포팅됨 - 사람 리뷰 필요(CLAUDE.md: 리뷰 없는 커밋 금지)
public Map<String, Object> fPLA040QrySelectDetail(Map<String, Object> request) {
	Map<String, Object> responseData = new java.util.HashMap<>();
	try {
		Map<String, Object> paramMap = request;
		...
		// FIXME(원본 버그): 원본의 requestData.putFieldMap(paramMap) 동작을 그대로 유지할 프레임워크 객체가 없어 생략됨

		Object rs = store.dPLA04002(request).get("DETAIL_LIST");
