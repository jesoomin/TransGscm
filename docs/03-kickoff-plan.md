...
- **멘토 코멘트 미반영 항목 재점검 + 조회 전용 가정 노출(2026-09-04)** — 사용자가 "멘토 의견 중
  ...
  **검토했지만 "지금 하면 안 된다"고 판단한 것(이유 포함)**:
  ...
  - **MCP 도입** — 리포지토리에 MCP 흔적은 하나도 없다(2026-09-04 시점). 도입하지 않기로 판단한
    이유: ① MCP의 핵심 가치는 모델이 도구를 동적으로 고르는 것인데 CLAUDE.md/멘토 §I가 똑같이
    "완전 자율 탐색형 에이전트를 만들지 않는다(1,416회 반복엔 고정 파이프라인이 낫다)"고 못 박았다
    ② 진짜 병목(유형 다양성, 실행 가능한 검증) 중 아무것도 해결하지 못한다 ③ **미루는 비용이 0이다**
    - `agents/*.py`가 이미 Streamlit 의존 없는 순수 함수라 나중에 얇은 어댑터로 감싸면 된다. 나중에
    한다면 범위는 **읽기 전용 조회**(nctRid 조회, 영향도 질의, 미사용 함수)로 한정하고 변환 실행은
    노출하지 않는다.
    **정정(2026-09-05) — 이 조건 그대로 실제로 만들었다.** `agents/mcp_server.py`(읽기 전용 4종:
    `impact_of_method`/`unused_methods`/`duplicate_methods`/`nctrid_map`, 변환 실행은 노출 안 함)
    + `agents/mcp_smoketest.py`(실제 MCP 클라이언트로 연결해 검증). 위 3가지 판단 근거와 상충하지
    않는다 - "미룬다"였지 "안 한다"가 아니었고, 미룬 조건(범위=읽기 전용)을 그대로 지켰다. 이
    항목이 위 "안 하기로 결정" 문단에 그대로 남아 있어 나중에 읽는 사람이 오해할 수 있어 정정만
    남긴다(문단 자체는 당시 판단 기록으로 보존).
...
- **자체 벤치마크 구축 — 정답키 대조 채점(2026-09-05, 멘토 §H)** — 멘토 §H가 요구한 "내부 벤치마크를
  먼저 만들어야 개선 여부를 측정할 수 있다"에 대한 응답. 그동안 탐지 기능은 "돌려보니 N건 나왔다"까지만
  말할 수 있었다 - **정답을 몰라 그 N건이 맞는지, 놓친 게 몇 건인지 말할 수 없었다.**
  `PLA081-110_migration_sample`(30화면 × 5도메인 LOG/MGT/PLN/PRD/QUL, P/F/D java + xsql 120파일)이
  정답키 2종을 함께 제공한다: 의도적으로 심은 결함 10건, 화면×멤버별 중복 유형 330행/78그룹.
  `agents/benchmark.py`(신규)가 우리 탐지기를 이 정답키와 대조해 재현율·오탐을 수치로 낸다(LLM 미사용,
  결정론적 대조라 회귀 측정에 그대로 쓸 수 있다).

  **실측 ①원본 결함 탐지: 10건 중 5건 (50%), 오탐 0건**

  | 분류 | 결과 | 놓친 것 |
  |---|---|---|
  | XSQL_SYNTAX | 2/2 | — |
  | XSQL_LOGIC | 1/1 | — |
  | JAVA_REFERENCE | 1/2 | 클래스명 오타(`FPLA100X`) |
  | JAVA_SYNTAX | 1/4 | 파라미터명 누락 · 세미콜론 누락 · import 누락 |
  | JAVA_LOGIC | 0/1 | `lookupDataUnit`을 `lookupFunctionUnit`으로 오용(컴파일은 통과, 런타임 실패) |

  잡아낸 5건은 중괄호 불일치, 존재하지 않는 메서드 호출, 없는 statement id 참조, XML 파싱 오류 2건이다.
  **놓친 5건이 곧 다음 보강 목록**이며, 정답키를 보고 규칙을 역으로 맞추지 않았다(그러면 측정 도구가
  아니라 과적합 대상이 된다).

  **부수 발견 — 모든 파일에 뜨는 신호는 결함 신호가 아니다**: 첫 실행에서 "정답키에 없는 파일 28건 탐지"가
  나왔는데 전부 `REMAPRESULTS_DROPPED` 하나였다. XSQL 30개 중 결함을 심은 2개를 뺀 정확히 28개 - 즉 모든
  파일에 균일하게 뜨는 **변환 정보성 알림**이지 원본 결함이 아니다. 결함 채점에서 제외해 오탐 0건이 됐다.

  **실측 ②중복 함수 탐지 — 정규화 수준에 따른 실제 트레이드오프**

  | 방식 | 완전중복 재현율 | 고유 오탐 | 탐지 그룹 |
  |---|---|---|---|
  | Type-1 (본문 원문 해시, 기존) | 105/210 (**50%**) | 0/60 (**0%**) | 6 |
  | Type-2 (화면 ID 정규화, 추가) | 209/210 (**100%**) | 29/60 (**48%**) | 17 |

  Type-1이 정확히 절반에서 멈추고 놓친 목록이 전부 `f*`인 게 단서였다 - F 메서드는 본문에 자기 화면의
  D 클래스명(`DPLA081` vs `DPLA082`)을 담고 있어 복붙된 같은 로직도 해시가 갈린다. 화면 ID를 자리표시자로
  치환하는 Type-2 정규화(클론 탐지의 표준 방식)를 넣으니 재현율이 두 배가 됐지만 **고유 판정에 48% 오탐이
  생겼다**. 어느 한쪽이 정답이 아니라 **재현율↔정밀도 트레이드오프**이므로 두 수치를 항상 함께 보고한다 -
  이 기능은 애초에 자동 삭제가 아니라 "검토 후보 제시"라 재현율 쪽에 무게를 두되, 신뢰 수준(확실/후보)을
  구분해 보여주는 게 맞는 설계다.

  **세트 선택 시 주의**: 이 세트에는 `.bizunit`이 없어 **nctRid가 0건**으로 잡힌다. "nctRid 자동 확정률
  100%"는 `sample_data/legacy-u-pla001-050/` 기준 수치다 - 지표를 보고할 때 어느 세트 기준인지 반드시
  함께 적는다.
...
- **PLA046 정적검증 실패 원인 규명 + 원본 XML 따옴표 결함 자동교정(2026-09-09)** — 사용자가
  "PLA046이 정적검증을 통과 못 한 게 원본 문제인지 변환 오류인지 분석해달라"고 요청. 위
  2026-09-08 항목 4번이 진단만 하고("줄 번호를 짚어준다") 넘어갔던 `<insert id="I005
  parameterClass="map">`류 결함(닫는 따옴표 누락, DPLA046 902/933/1032행)을 재확인한 결과 **원본
  결함이 맞다**(047과 같은 부류 - 원본이 깨져도 스킵하지 않고 정정 후 진행하는 원칙 그대로
  적용). 이 모양은 해석이 모호하지 않아(속성값 중간에 `이름="`가 다시 나오는 건 다음 속성이
  시작된 것 말고는 설명할 방법이 없음) `converters.repair_unterminated_attr_quotes()`를 추가해
  자동 교정하고 `ORIGINAL_XML_ATTR_QUOTE_FIXED` 경고로 남긴다.

  **연쇄 버그 발견**: 같은 결함이 `skeleton_gen.extract_xsql_stmt_kinds()`(Store가 select/
  insert/update/delete 중 무엇을 생성할지 XSQL 태그로 판정하는 함수)도 독립적으로 정규식
  파싱하다 깨뜨려서, XSQL엔 `<insert>`로 정의된 `dPLA04618/19/26`이 "미지원 DB 동사"로
  오탐되고 있었다 - Mapper 쪽만 고쳐서는 안 맞춰지는 두 번째 소비처였다. 여기도 같은 교정을
  적용. **부수 발견**: `unsupported_db_verbs()`가 2026-09-08 DML 지원 추가 이전 기준
  (`SUPPORTED_DB_VERBS = {"Select"}`, 위 §7의 "화이트리스트 없이" 서술과 달리 실제로는
  화이트리스트였다)에 멈춰 있어 XSQL이 확정해주는 insert/update/delete까지 전부 "미지원"으로
  보고하고 있었다 - `d_xsql_text`를 받아 `extract_xsql_stmt_kinds()`로 교차 확인하도록 확장
  (`agents/conversion_plan.py`도 동일 반영). **실측(DPLA046 재검증)**: Store가
  selectOne 14·insert **7**(4→7)·update 4·delete 1로 정정됐고(2026-09-08 표의 "insert 4"는
  이 결함 때문에 3건이 덜 잡힌 값이었다), 미지원으로 정확히 좁혀진 건 진짜 프로시저 3건뿐.
  PLA045/047~050 전체 재검증(pytest + 직접 실행) 결과 이 버그 클래스의 재발 없음 확인.

- **D 메서드 1개가 statement 여러 개를 순서대로/조건부로 실행하는 패턴 — 규칙 기반 자동화는
  안 하되 AI 추천(opt-in)으로 보완(2026-09-09, 아래 후속 항목 참고)** — 위 재검증 과정에서
  별개의, 더 오래된 한계를 발견: `extract_d_stmt_ids()`는
  메서드당 **첫 번째** db* 호출만 D 메서드명에 매핑한다(2026-09-08 주석에 이미 명시된 설계).
  실측 사례 3건 — PLA046 `dPLA04625`(프로시저 P003 실행 후 `dbSelect("S099", ...)`, 무조건
  순차), PLA045 `dPLA04505`(프로시저 P001 실행 후 **`if("SUCCESS".equals(...))` 조건부로만**
  P002, 마지막에 S099), PLA046의 한 D 메서드(U002 실행 후 U003, 무조건 순차) — 에서 두 번째
  이후 statement(S099, U003)가 Mapper.xml에 원본 id 그대로 남아 `STMT_ID_MAP_MISSING`으로만
  잡히고 "왜"는 안 보였다.

  **판단: 이 매핑을 자동으로 완성하는 Store 코드 생성기를 만들지 않는다.** 이유:
  ① `dPLA04505` 사례가 보여주듯 두 번째 호출 여부 자체가 **조건부 분기**(`if`)일 수 있다 -
  D 계층은 원래 순수 데이터 접근이어야 하는데(P/F/D 계층 분리 전제) 이건 이미 F 계층 성격의
  업무 로직이 D 계층에 섞인 경우라, 규칙 기반으로 찍어내면 CLAUDE.md "확인되지 않은 로직을
  추측으로 만들지 않는다" 원칙과 정면으로 부딪힌다. ② 무조건 순차 호출(dPLA04625, U002/U003
  사례)조차 "한 Store 메서드가 여러 Mapper statement를 어떤 반환값 규칙으로 묶을지"는 업무
  판단이 필요해 나머지 규칙 기반 변환(1 D메서드 = 1 SQL문 가정)과 전제가 다르다. ③ 실측
  빈도가 6개 실화면 중 3건뿐으로 극히 드물다 - 희귀 케이스를 위해 P 계층의
  `P_ORCHESTRATION_PORT_REQUIRED` 같은 별도 LLM 포팅 경로를 D 계층에도 새로 뚫는 건 지금
  투자 대비 과하다.

  **대신 반영한 것**: 자동화가 아니라 **진단 품질 개선**만. `skeleton_gen.
  extract_secondary_stmt_owners()`(신규)가 "이 statement, 사실 어느 D 메서드가 부르고는
  있다(첫 호출이 아니라서 자동 매핑만 안 됐을 뿐)"를 찾아 `finalize_mapper_document()`의
  `STMT_ID_MAP_MISSING` 메시지에 그 메서드명을 실어준다 - 매핑 자체는 여전히 사람이 완성해야
  하지만 "어디를 봐야 하는지"는 이제 즉시 알 수 있다. 진짜 미참조 statement(PLA046의 S010 -
  XSQL엔 있지만 Java 어디서도 안 부름)는 이 컨텍스트가 없어 기존 일반 메시지 그대로 남는다 -
  두 경우를 구분해서 보여준다. **검증**: PLA046/045 실 소스로 S099→dPLA04625,
  U003→dPLA04614가 정확히 귀속되고 S010은 일반 메시지 그대로 나옴을 확인, pytest 회귀 없음.

  **후속(2026-09-09, 사용자 지적) — "자동화 안 함"과 "AI 추천 안 함"을 같은 걸로 착각했었다.**
  위 판단의 근거(①②)는 전부 "**규칙 기반**으로 추측해 찍어내면 안 된다"는 것이었는데, 이
  프로젝트엔 정확히 이런 상황을 위한 별도 카테고리가 이미 있다 - **AI 추천**
  (`react_variant.py`): LLM이 제안하고, 원본과 대조해 지어낸 부분은 BLOCKER로 걸러 사람이
  채택 못 하게 막고, 결과를 채택할지는 전적으로 사람이 검토 후 결정하는 opt-in 산출물. 규칙
  기반 파이프라인(위 ①②가 막으려던 대상)은 전혀 건드리지 않는다. 그래서 같은 패턴을 이
  문제에도 적용했다 - `chatui/d_orchestration_variant.py`(신규). **분리 원칙은 유지**: "이
  D 메서드가 실제로 부르는 statement 목록"은 이미 결정론적으로 아는 사실이라 Store 분할은
  여전히 **규칙 기반**(LLM 미사용, `build_split_store_methods()`)으로 만들고, LLM은 오직
  "그 순서/조건 구조를 Service 코드로 어떻게 옮길지"(원본을 읽어야 아는 부분)에만 쓴다. LLM이
  목록에 없는 Store 메서드를 지어내면 `D_ORCHESTRATION_INVENTED_CALL` BLOCKER로 막는다
  (react_variant.py의 REACT_VARIANT_INVENTED_FIELD와 동일 원칙). **실 LLM 검증(3사례 전부)**:
  PLA046 `dPLA04625`(프로시저+조회, 무조건 순차) - 프로시저는 TODO로 남기고 조회만
  `store.dPLA04625_S099(...)`로 정확히 교체, 지어낸 호출 0건. PLA046의 U002/U003 메서드(순수
  DML 순차) - 두 update를 정확한 순서로 분할 호출, 이슈 0건. PLA045 `dPLA04505`(프로시저 2개,
  **두 번째가 조건부**) - `if ("SUCCESS".equals(sResult1))` 분기 구조를 그대로 보존하면서
  프로시저 2건은 TODO, 조회만 교체 - 가장 우려했던 조건부 케이스도 원본 분기를 안 건드리고
  올바르게 처리했다. UI는 기존 "🎨 AI 추천"과 나란히 "🔀 D계층 재구성" 탭/expander로
  추가(단일 화면·배치 상세보기 양쪽), 헤드리스 `AppTest` 부팅 확인, pytest 회귀 없음.

- **외부 리뷰(AlphaTrans/ReCodeAgent/MatchFixAgent/Copilot Modernization/AWS Transform 재검토)
  반영 — 명시적 계약 객체 + Store 카디널리티 결함 발견·수정(2026-09-10)** — 사용자가 외부 검토
  의견(§A~§J와 같은 프레임워크를 다시 평가한 2차 분석)을 갖고 와서 "이 프로젝트에 적합한지
  판단하라"고 요청. 결론: 제안된 아키텍처(Domain IR/Contract Compiler/Translator/Differential
  Validator/Repair Router/Verdict/Human Approval)의 대부분은 이미 다른 이름으로 구현돼 있었다
  (Domain IR = `agents/nctrid_graph.py`+`NCTRID_MAP`, Contract Compiler = `conversion_plan.py`+
  스켈레톤 우선 생성, Differential Validator = `diff_test.py`+`equivalence_test.py`, Translator
  = `port_one_screen_method`, Repair = `repair_gate_node`). 실제로 비어 있던 건 하나 -
  **콜리 의존 정보를 프롬프트에 자유 텍스트(`_callee_note`)로만 주던 것을 명시적 JSON 계약
  객체로 바꾸는 것**(§5 제안, AlphaTrans의 "메타데이터를 프래그먼트마다 명시적으로 전달" 원칙).

  **구현 중 이 계약을 정확히 채우려다 실제 결함을 하나 발견했다 - Store의 카디널리티(단건/다건)
  가정이 원본과 어긋나는 사례가 광범위하게 있었다.** `skeleton_gen.py`는 select statement를
  전부 `sqlSession.selectOne(...)`(단건, `Map<String,Object>`)로 생성하는데, 실제 PLA047의
  `dPLA04701`(S001)은 `WHERE PLN_YW=#PLN_YW# AND USE_YN='Y' AND PLN_TYP_CD != 'BSE'`로 유일키
  조건이 없고 `ORDER BY`까지 있는 다건 조회이며, F 코드도 `du.dPLA04701(...).getRecordSet(
  "REV_LIST")`로 받은 recordset을 색인 없이 그대로 `responseData.putRecordset("REV_LIST", rs)`로
  통째로 넘긴다 - 즉 원본은 이미 다건으로 설계했는데 TO-BE Store는 단건만 지원하는 `selectOne`을
  쓴다. 실제로 2행 이상 나오면 MyBatis가 런타임에 `TooManyResultsException`을 던진다(컴파일도,
  기존 정적 검증도, L3 동등성 하네스도 못 잡는다 - 하네스가 Store를 스텁으로 우회하기 때문).
  PLA047 6개 D 메서드 중 5개가 이 패턴이었고, PLA045~050 6개 실화면 전체에서 재현됐다(화면당
  4~10건) - **PLA047 하나의 우연이 아니라 이 코드베이스의 "recordset 조회" 화면 대부분에 걸린
  구조적 결함**이었다.

  **처음 잡은 판정 규칙은 틀렸다(자체 발견, 즉시 정정)** - "결과를 `getRecordSet(...)`으로
  꺼내면 다건"으로 잡았더니 `dPLA04702`(PIVOT_LIST)가 오탐이었다. 실제로는 `getRecordSet`으로
  꺼낸 뒤 `rdPivot.get(0, "필드명")`처럼 **고정 인덱스 0만 반복 접근**하고 있어 원본이 단건으로
  다루는 사례였다 - `getRecordSet` 자체는 NEXCORE에서 결과를 꺼내는 유일한 API라 다건/단건을
  구분하지 못한다(둘 다 이 API를 거친다). 진짜 신호는 **꺼낸 변수를 그 뒤에 어떻게 쓰는가**다:
  색인 없이 `putRecordset(...)`으로 통째로 넘기면 다건, `.get(고정숫자, ...)`로만 접근하면
  단건. `skeleton_gen.infer_d_method_cardinality()`(신규)가 메서드 단위로(파일 전체를 한
  텍스트로 보면 변수명이 메서드마다 재사용돼 다른 메서드의 접근 패턴과 섞인다) 이 두 신호를
  본다 - 재검증 결과 PLA047: 5/6건 정확히 재현(dPLA04702 제외), 오탐 0건.

  **자동으로 고치지 않는다.** Store 반환 타입을 `List<Map<String,Object>>`로 바꾸는 건 Store만이
  아니라 그 위 Service/Api 시그니처까지 연쇄적으로 바뀌는 더 큰 판단이라(단순 규칙 적용이 아니라
  아키텍처 결정), 이번엔 **드러내는 것까지만** 했다: `skeleton_gen.py`가 Store 생성 시점에
  `STORE_CARDINALITY_MISMATCH` BLOCKER를 메서드에 귀속시켜 남기고, `_callee_note`(포팅 프롬프트)
  도 이 결론을 그대로 읽어와 LLM에게 "이 콜리는 원본이 다건인데 Store는 단건이다 - 임의로
  봉합하지 말고 FIXME만 남겨라"를 명시한다. **단일 진실 공급원 원칙**: 카디널리티 판정은
  `skeleton_gen.py`에서 한 번만 하고, `agents/workflow_graph.py`는 그 결론(`skel_issues`의
  `STORE_CARDINALITY_MISMATCH`)을 그대로 읽기만 한다 - 처음엔 워크플로 쪽에서 F/P 텍스트를 보고
  독립적으로 다시 판정했는데, DML(insert/update/delete) 콜리까지 "Store가 Map을 반환한다"는
  틀린 문구를 달 뻔한 걸 실측 중 발견해(Store는 DML엔 이미 `int`를 반환하므로 이 판정 대상이
  아님) `skel_issues` 조회로 바꿔 해소했다 - 두 곳이 각자 판정하면 반드시 어긋나는 경우가
  생긴다는 걸 이 프로젝트에서 이미 여러 번 겪었다(AI 추천, Maven 에러 메시지 등과 같은 교훈).

  **실 LLM 검증**: PLA046 `fPLA046QryInsertMainList` 포팅에서 LLM이 계약의 `dPLA04606`
  카디널리티 불일치를 인지하고 `// FIXME(카디널리티 불일치, 사람 확인 필요): 원본은 dPLA04606
  결과를 다건(recordset) MAIN_LIST로 받아 putRecordset/get(0, ...) 처리함. 현재 store는 Map
  단건 반환 계약이라 원본 다건 전제를 그대로 재현할 수 없음.`을 정확히 남겼고, 같은 계약의 DML
  콜리(dPLA04607/13/17/18/19/26)에는 이 주석을 달지 않았다(DML 판정 제외가 실제로 작동함).
  전체 6화면 dry-run 회귀 없음(잔여 BLOCKER 45건 동일), pytest 통과.

  **§3(Verdict를 별도 아티팩트로) 검토·비판 — 지금은 만들지 않는다.** 제안은 정적 검증 +
  차등/동등성 검증 결과를 하나로 합쳐 "APPROVED/NEEDS_REVIEW/REJECTED" 같은 단일 판정을 내는
  별도 단계를 두자는 것이었다. 비판적으로 보면:
  1. 이미 사실상 이 역할을 하는 곳이 있다 - `handoff_report.py`가 BLOCKER/WARNING 개수와
     "반드시 처리"/"확인 필요" 절을 사람이 읽을 순서로 이미 구성해준다. 별도 Verdict 필드를
     새로 만들어도 사람이 보는 정보는 같다(구조화된 JSON이냐 정리된 markdown이냐 차이일 뿐).
  2. 지금 아키텍처에서 "승인하고 저장" 게이트 자체가 이미 사실상의 Verdict 단계다 - 사람이
     그 자리에서 handoff_report.md를 보고 판정을 내린다. 자동 Verdict를 그 앞에 끼워 넣어도
     사람이 하는 일이 줄지 않는다(같은 근거 데이터를 다시 본다).
  3. 진짜 가치가 생기는 조건은 **차등/동등성 테스트가 메인 파이프라인의 필수 단계가 될 때**다
     - 지금은 `diff_test.py`/`equivalence_test.py`가 opt-in 별도 스크립트라 정적 검증 결과와
     같은 실행에서 동시에 안 나온다. 그 전에 Verdict를 만들면 "이미 있는 BLOCKER 집계를 다른
     이름으로 한 번 더 세는" 것 이상의 가치가 없다.
  **결론**: 만들지 않는다. 차등/동등성 테스트를 메인 파이프라인 필수 단계로 승격하기로 결정하는
  시점이 오면, 그때 Verdict는 새 문서를 만드는 게 아니라 `handoff_report.py`의 기존 요약 줄에
  "정적 검증 + 차등 테스트 종합 판정"을 확장해 넣는 방식으로(중복 구현 금지 원칙) 붙인다.

- **소급 재검사(2026-09-10) - 이미 `pilot/`에 저장된 화면도 이 결함을 안고 있다.** `STORE_CARDINALITY_MISMATCH`는 오늘 새로 추가한 검사라, 그 이전에 승인·저장된 PLA045~050(현재 `pilot/`에 있는 전부)은 이 검사 없이 생성됐다. 저장된 화면을 건드리지 않고 AS-IS 소스로만 `generate_skeletons()`를 다시 돌려(읽기 전용) 확인한 결과, **6화면 전부 이미 이 리스크를 안고 저장돼 있다**(PLA045 5건·046 10건·047 5건·048 4건·049 5건·050 4건 - 049/050의 `dCommonCodeQry`/`dHistoryQry` 2건은 실제 파이프라인에서는 공통 Store로 라우팅돼 이 검사를 안 타므로 표본 스크립트의 과대계상). 파일을 다시 쓰지는 않았다(재생성은 별도 승인 없이 하지 않는다는 원칙) - 사람이 `pilot/`을 다음에 손댈 때 이 목록을 참고 자료로 남긴다.

## 추가로 고려할 만한 다음 단계 (2026-09-10, 사용자 요청 - 우선순위 판단 포함)

1. **(추천, 미착수) STORE_CARDINALITY_MISMATCH용 AI 추천 확장** — `d_orchestration_variant.py`와
   같은 패턴(LLM 제안 + 원본 대조 검증 + opt-in + 사람 채택)으로, Store를 `selectList`/
   `List<Map<String,Object>>`로 바꾸고 그 위 Service/Api까지 연쇄 반영하는 **대안**을 제안받는
   기능. 지금은 결함을 드러내기만 하고 고치는 건 전부 사람 몫인데, 이미 검증된 AI 추천
   인프라(원본 필드/호출 대조 검증)를 재사용하면 큰 신규 위험 없이 "사람이 검토할 초안"까지
   자동으로 만들어줄 수 있다. **우선순위 근거**: 6화면 전부, 화면당 4~10건이라 파일럿(Phase 3)에
   들어가면 반복 빈도가 가장 높은 수리 대상이 될 가능성이 큼.
2. **(검토 필요) 차등/동등성 테스트를 메인 파이프라인 필수 단계로 승격** — 위 Verdict 판단(§)이
   의존하고 있던 전제. 지금은 opt-in이라 저장 전 6~7단계(교차분석/Maven)에는 안 끼어 있다.
   승격하면 Verdict뿐 아니라 A2(정적 검증 통과율) 정의 자체를 "정적+동등성 합산"으로 넓힐지도
   같이 결정해야 한다(범위가 커서 사람 결정 필요 - 지금 코드로 바로 착수하지 않음).
3. **(검토 필요) DML(insert/update/delete) 콜리에도 카디널리티류 위험이 있는지 점검** — 이번
   발견은 select(다건 조회)에 한정했다. update/delete가 "정확히 1행만 바뀌어야 하는" 원본
   전제를 갖고 있는데 실제 WHERE 조건이 다건에 걸리는 경우도 이론적으로 같은 계열의 위험이다
   (MyBatis의 update/delete는 예외를 던지지 않고 조용히 여러 행을 바꾼다는 점에서 오히려 더
   위험할 수 있음 - 다만 아직 실제 사례로 확인된 바 없어 추측 단계, Phase 3 파일럿에서 CRUD
   화면이 더 들어오면 재확인).
4. **(보류) 파일럿 유형 다양성 추가 확장** - 세트 C(PLA045~050)가 세트 A(1유형 복제)보다는
   낫지만 여전히 "경영분석/PLA" 1개 업무그룹뿐이다. Phase 3의 "전체 화면 구조적 클러스터링"이
   실제로 시작되기 전까지는 세트 C 하나로 일반화하지 않는다(§1.5 평가지표 재평가 문서에도 같은
   경고 있음). **사용자 지시(2026-09-10): "4번 빼고 검토하고 추가 적용" - 1~3번 구현 완료**, 아래 참고.

### 1~3번 구현(2026-09-10)

**1. STORE_CARDINALITY_MISMATCH용 AI 추천 — `chatui/cardinality_fix_variant.py`(신규)**.
`d_orchestration_variant.py`와 같은 분리 원칙: Store 수정(`selectOne`→`selectList`,
`Map<String,Object>`→`List<Map<String,Object>>`)은 이 D 메서드가 다건이라는 게 이미 결정론적으로
확인된 사실이라 **규칙 기반**(`build_fixed_store_method()`, LLM 미사용)으로 만들고, 그 결과를
소비하는 **Service 코드 적응만 LLM 1회**로 만든다(`recommend_cardinality_fix()`). 원본과 무관한
Store 호출을 지어내면 `CARDINALITY_FIX_INVENTED_CALL` BLOCKER로 막는다. UI는 기존 "🎨 AI 추천"/
"🔀 D계층 재구성"과 나란히 "📐 카디널리티 수정" 탭/expander로 추가(단일 화면·배치 상세보기
양쪽). **실 LLM 검증**: PLA047 `dPLA04701`(REV_LIST, 순수 passthrough) 사례로 확인 - 현재
규칙 기반 코드가 `result.get("REV_LIST")`로 **엉뚱한 키를 다시 꺼내려던 별개의 기존 결함**까지
LLM이 `response.put("REV_LIST", result)`로 함께 바로잡았다(요구하지 않았는데 자연스럽게 고쳐짐,
프롬프트가 "그 외 로직은 바꾸지 마라"고 못박아뒀는데도 타입 변경의 직접 결과라 반영된 것으로
보임 - 우연한 개선이지 의도한 범위는 아니었다는 점을 밝혀둔다). 지어낸 호출 0건, 타입 반영 확인.
pytest·헤드리스 `AppTest` 부팅 확인.

**2. 차등/동등성 테스트를 메인 파이프라인 필수 단계로 승격 — `agents/workflow_graph.py`에
`equivalence_check_all` 노드 신설(Stage 6/8)**. `agents/diff_test.py`(SQL 계층, 실제 Oracle DB
필요)는 이번엔 제외하고, **DB 없이 javac/java만으로 도는 `agents/equivalence_test.py`(L3, F/Api
계층)만** 자동화 대상으로 좁혔다 - 그동안 이 하네스는 `python -m agents.equivalence_test`로
사람이 따로 돌리는 연구용 스크립트였고 어떤 파이프라인에도 안 물려 있었다. `run_screen()`이
평평한 디렉터리 이름 규칙(`asis_dir/F{screen}.java` 등)을 기대해서, `chatui/app.py`의
`_run_stage_6_7_preview()`와 같은 임시 디렉터리 격리 패턴(쓰고 실행하고 `finally`에서 항상
삭제)을 그대로 따랐다 - 저장 전에도 실행되고 실제 `pilot/`은 안 건드린다. **안전장치**: 내부
예외가 파이프라인 전체를 막지 않도록 이 노드에서 감쌌다(opt-in 스크립트였을 때와 달리 이제는
자동 단계라 여기서 죽으면 이후 저장까지 다 막힌다) - 실패해도 WARNING 기록 후 다음 단계로
진행. 파이프라인 전체 단계 수가 7→8로 늘어 로그의 모든 `STAGE N/7` 표시를 `N/8`로,
Stage 6(품질 스캔)·Stage 7(AI 추천)을 각각 7·8로 밀었다. **실측**: PLA047은 AS-IS 원본 자체의
브레이스 결함(`fPLA047QrySelectMainList`, 이미 알려진 원본 버그) 때문에 컴파일 실패로
정확히 잡혔고(신규 버그 아님, 기존에 알려진 결함이 이제 자동으로도 잡힌다는 뜻), **PLA048은
실제로 실행되어 15/24케이스(62.5%) 일치**를 자동으로 산출했다 - 이 수치는 이 통합 전에는
누구도 자동으로 본 적이 없는 새 정보다(다음에 PLA048을 다룰 때 남은 9케이스 불일치 원인을
살펴볼 필요가 있음 - 이번 작업 범위 밖이라 원인 분석은 안 함). 6화면 dry-run에서도 3/6화면
실행·오류 0건 확인(오류 없이 도는 것만 확인, 스텁 코드 대비 일치율 수치 자체는 참고용).
pytest·그래프 컴파일 확인 통과.

**3. DML 카디널리티 위험 점검 — 위험 없음, 결론이 다른 실질 개선으로 이어짐.** 실제 F/D 소스를
전수 확인한 결과 DML(insert/update/delete) 반환값(영향 행 수)을 특정 값과 비교하는 로직이
어디에도 없었다 - MyBatis의 `update()`/`delete()`는 `selectOne()`과 달리 행 수와 무관하게
예외 없이 반환하므로, select와 같은 "조용한 런타임 크래시" 위험은 **없다는 게 확인됐다**(추측이
아니라 실제 코드 대조로 결론지음). 대신 조사 중 별개의 실질 개선점을 찾았다 - 포팅 프롬프트가
콜리의 실제 Store 반환 타입(`int` vs `Map<String,Object>`)을 안 알려줘서 LLM이 DML 콜리를
select처럼 추측하다 컴파일 에러→수리 루프 낭비로 이어질 수 있었다. `skeleton_gen.py`의 D 메서드
생성 3개 분기(공통위임/DML/select) 전부에 `store_return_type` 필드를 추가하고,
`agents/workflow_graph.py`의 계약 JSON에 `return_type`으로 실어 보내도록 확장했다(1번 항목의
계약 객체 작업을 그대로 확장 - 새 메커니즘을 안 만듦). pytest·6화면 dry-run 회귀 없음.

### 최종 점검(2026-09-10, 사용자 요청 - "반영점 및 기존 로직상 문제 재점검") — 실제 결함 1건 발견·수정

2번(동등성 검증 파이프라인 승격)을 넣은 직후 다시 훑다가, **console 로그만 고치고 Streamlit UI는
안 고쳤다는 걸 발견했다.** `chatui/app.py`의 폴더 모드 진행률 표시(`_pipeline_progress_cb`,
`stage_placeholders`, `_STAGE_LABELS`)는 `agents/workflow_graph.py`의 콘솔 로그 단계 번호와는
**별개의, 자체 하드코딩된 7단계 체계**(0=계획~4=품질스캔~5=AI추천, 6~7=저장 전 교차분석/Maven
미리보기)를 쓰고 있었다 - 콘솔 쪽만 7→8단계로 고치고 이쪽은 그대로 뒀다. 그 결과:
`validate_all`이 끝나는 즉시 UI가 "4단계(품질·취약점 스캔) 진행 중"으로 표시하는데, 실제로는
그 사이에 수리 루프 + **새로 생긴 동등성 검증(javac 컴파일 포함, 수 초~수십 초)**이 아직
돌고 있어 - 사람이 "멈췄나?"라고 오해할 수 있는 상태였다. 실제 배치를 막는 크래시는 아니지만
"기존 로직상 문제"에 정확히 해당해 바로 고쳤다: `_STAGE_LABELS`에 4단계
"기능 동등성 검증(L3)"을 끼워 넣고 품질스캔(4→5)/AI추천(5→6)/교차분석(6→7)/Maven(7→8)을
전부 한 칸씩 밀었다(`stage_placeholders` 8→9칸), `_pipeline_progress_cb`에
`equivalence_check_all` 노드 전용 분기를 새로 추가해 실행 결과(화면 수·케이스 일치율)를 그
자리에서 바로 보여주도록 했다. 헤드리스 `AppTest` 부팅 확인 + 실 파이프라인 실행(PLA048)으로
콘솔 요약 표에 새 "기능 동등성(L3)" 행이 정확한 값으로 나오는 것까지 재확인.

이 외 점검한 것(문제 없음 확인): `agents/db.py`의 `upsert_conv_method`가 명시적 키워드 인자만
받아써서 `store_return_type` 같은 새 필드를 methods dict에 추가해도 DB 저장 경로가 깨지지
않음, `run_pipeline.py`/`log.summary()`의 BLOCKER 집계가 `validation_results`만 보고
`skel_issues`(이번에 추가한 `STORE_CARDINALITY_MISMATCH` 포함)는 안 봐서 기존 A2 지표 정의와
충돌 없음(의도된 기존 설계), 카디널리티 불일치와 D 계층 다중 statement 두 대상 집합이 6화면
전부에서 실제로 겹치지 않음(두 AI 추천 탭이 같은 메서드를 동시에 건드릴 위험 없음), 모든 신규/
수정 모듈(chatui 쪽 전부 + agents/workflow_graph.py) import 정상.

**이 세션 작업 기준으로는 마무리 가능한 상태다** - pytest·헤드리스 UI 부팅·실 LLM 검증·실
파이프라인 실행(dry-run + 실행 각 1회 이상) 전부 통과했고, 발견된 진짜 문제(위 UI 단계 표시
불일치)는 고쳤다. 다만 이건 "이번 세션에서 손댄 범위"의 마무리이지 프로젝트 전체의 완료를
뜻하지 않는다 - Phase 3(파일럿 20~30화면, 아직 미착수)이 이 프로젝트의 다음 실질 관문이고,
PLA048의 62.5% 불일치 원인처럼 이번에 새로 드러난 것 중 조사가 필요한 항목도 남아 있다.

### UI 개선 — `st.status()`로 "에이전트가 단계별로 생각하며 진행" 느낌 추가(2026-09-10, 사용자 요청)

폴더 모드 파이프라인 실행 버튼을 눌렀을 때의 진행률 표시를 `st.empty()` 라인 갱신 방식 그대로
두되(이미 검증된 화면별 체크리스트라 로직은 안 건드림), 그 바깥을 `st.status(...)`로 감쌌다.
실행 중에는 스피너 + "🤖 N단계: ~~ 중... (진행/전체)" 한 줄짜리 현재 활동 라벨이 실시간으로
바뀌고, 끝나면 자동으로 접히며 "✅ 파이프라인 완료"로 바뀐다 - Claude Code 자체의 도구 호출
패널이 접히는 것과 비슷한 느낌을 노렸다. `_pipeline_progress_cb`의 각 노드 분기마다
`pipeline_status.update(label=...)` 한 줄씩만 추가했다(표시 전용이라 실패해도 파이프라인 자체엔
영향 없음). **검증**: 격리된 최소 재현으로 `st.status()` 안에서 중첩 콜백이 `.update()`를 호출하는
패턴 자체가 Streamlit(1.61.1)에서 정상 동작함을 먼저 확인한 뒤 반영, 헤드리스 `AppTest` 부팅 확인,
pytest 회귀 없음. 실제 클릭 흐름(스피너가 도는 모습 자체)은 브라우저가 없어 시각적으로는 확인
못 했다 - 이 프로젝트의 다른 Streamlit UI 작업과 같은 한계.

### 추론 로그 강화 — 모델의 판단을 콘솔로 되돌려 출력(2026-09-10, 사용자 요청)

계기는 시연 영상이다. 사용자 요청: *"로컬 터미널에서 LLM이 생각하는 모드나 로직상 구현하는 걸
보여줄 수 있는지."* 제출 가이드가 *"단순 로그는 평가에 불리 — Planning, Self-Correction,
Tool Call 로그가 보여야 한다"*고 못 박고 있어 실제 배점과 직결된다.

**핵심 판단 — 지어내지 않고 보여줄 방법이 이미 있었다.** 포팅·수리 프롬프트가 모델에게
**본문 첫 줄에 `// AI 변경 요약:` 한 줄을 남기라고 이미 요구**하고 있다(2026-09-04 "변경 사유"
항목). 그 문장은 우리가 쓴 해설이 아니라 **모델 자신의 말**이므로, 콘솔에 그대로 되돌려 출력하면
`reasoning_log.py`의 "없는 추론을 지어내지 않는다" 원칙을 깨지 않고 사고 과정을 보여줄 수 있다.

**반영한 것 4가지**:
1. **`THINK` 이벤트 신설**(`reasoning_log.think()`) — 모델이 밝힌 근거를 **따옴표로 인용해** 출력한다.
   로그의 나머지가 "코드가 내린 결정"인 것과 시각적으로 구분된다. 모델이 스스로 남긴
   `FIXME`/`TODO`도 함께 싣는다 — "사람이 봐야 할 것"을 모델이 직접 표시한 것이라서다.
   `_extract_ai_rationale()`이 주석을 못 찾으면 **아무것도 출력하지 않는다**(대신 문장을 만들지 않음).
   최초 포팅(`port_one_screen_method_node`)과 ToT 수리 후보(`repair_candidate_node`) 양쪽에 붙였다.
2. **계획 → 도구 호출 인과를 로그로 드러냄** — `plan_all`이 "도구 호출 계획"(규칙 변환기 N건 ·
   LLM Gateway M건 · 검증기 전건 · 수리는 실패 시에만)을 확정해 출력하고, `dispatch_ports_all`이
   "1단계 계획이 지목한 N건에만 호출한다"를 명시한다. **모델이 무엇을 부를지 스스로 고르지 않는다**는
   고정 파이프라인의 성질이 로그 두 줄로 증명된다.
3. **호출별 소요 시간** — 응답 수신 줄에 응답 길이와 초를 함께 찍는다.
4. **줄바꿈을 터미널 표시 폭 기준으로** — 글자 수로 자르면 한글이 두 칸을 차지해 실제 줄 길이가
   두 배가 된다(76자로 잘랐더니 화면에서 140칸을 넘어 접혔다). `unicodedata.east_asian_width`를
   반영했다. 영상에서 줄이 접히면 로그를 못 읽는다.

**이 과정에서 찾은 실제 결함 1건 — 녹화 중 파이프라인이 죽을 수 있었다.** Windows 기본
코드페이지(cp949)에서 로그의 박스 문자(`┌─│└`)와 em dash가 `UnicodeEncodeError`를 내며
**파이프라인 전체를 중단시킨다**(실측 재현). 표시용 로그가 실행을 멈추게 둘 수 없어
`log.enable()`이 stdout을 UTF-8(`errors="replace"`)로 재설정하도록 했다. 녹화 환경이 조금만
달랐어도 촬영 도중 터졌을 결함이다.

**실 LLM 검증(PLA047 1화면, 저장 없음)**: 4건 전부 `THINK`가 정상 출력됐다. Api 계층 3건은
`TODO(응답 규약 미확정)`을, F 계층 1건은 **`FIXME(원본 버그)` 2건**(쓰이지 않는 `fu` 변수,
if 블록 중괄호 누락)을 모델이 스스로 표시했다 — 요구하지 않은 원본 결함 지적이다.
dry-run 2화면 완주·pytest 통과로 회귀 없음 확인.

**부수 실측 — 시연 세트 선택 근거**: PLA047의 `fPLA047QrySelectMainList`는 프롬프트 29,483자 →
응답 37,144자로 **단건에 96.6초**가 걸린다. 5분 영상에 담기지 않으므로 녹화는 세트 B
(`PLA081-110_migration_sample`, 5화면 약 30초)로 한다 — `docs/weekly/시연영상-스크립트.md`에 반영.

### 선행 연구 참조 정리(2026-09-10, 사용자 요청)

AlphaTrans/ReCodeAgent 등 참조가 이 문서와 @docs/06-mentor-feedback.md 곳곳에 흩어져 있어
@docs/10-prior-art.md로 모았다. **새 주장을 하지 않는다** — 이미 코드로 확인해 기록해 둔 차용·기각
판단을 출처별로 재배치하고, 각 항목에 ①무엇을 차용했나(구현 파일까지) ②무엇을 기각했나와 이유
③이 프로젝트가 더한 것을 붙였다.

**정리하다 발견한 수치 불일치**: AlphaTrans 수치가 두 갈래다. @docs/06-mentor-feedback.md는
99.1% / 25.8%로 인용했는데, 이 문서 2026-09-04 항목에 **원문을 직접 확인한 값은 96.40% / 25.14%**로
적혀 있다. 결론(문법과 기능 사이의 큰 간극)은 같지만 **직접 확인한 값을 쓴다** — 인용 수치를
검증 없이 옮기지 않는다. 발표 산출물에도 이 기준을 명시했다.

### AI Master 제출 산출물(2026-09-10)

- `docs/weekly/assets/submission-worksheet.html` — 플랫폼 8개 태스크 입력칸에 그대로 붙여넣는
  **마크다운 기호 없는 순수 텍스트** 블록 모음(플랫폼에서 md가 깨지는 문제 대응) + 시연 대본에서
  역산한 장표 초안.
- `docs/weekly/assets/최종발표장표.pptx` — **제출 가이드 규격은 표지 + 3페이지(총 4슬라이드)**임을
  확인하고 공식 템플릿에 채운 최종본. 슬라이드 2의 아키텍처 다이어그램은 도형으로 직접 그렸다
  (가이드가 허용). 수치는 전부 @docs/weekly/00-종합산출물.md 실측값.
- **가이드 자체의 오류를 따르지 않았다**: 장표 가이드 예시문이 MCP를 "Model-based Coherent
  Planning"으로 잘못 적어놨다. MCP는 **Model Context Protocol**이고 이 프로젝트엔 실제 구현
  (`agents/mcp_server.py`, 읽기 전용 4종)이 있다. 올바른 정의를 쓰고 "표면을 갈랐다"는 설계 판단을
  선택 이유로 적었다 — 틀린 용어를 따라 쓰면 오히려 감점 요인이다.

### 규칙 기반 위임 메서드 개명 제거 — BLOCKER 10건 중 8건이 우리가 만든 것이었다(2026-09-12)

시연 대본이 인용한 로그를 다시 대조하다가 발견했다. 정적 검증이 잡던 BLOCKER 10건 중 **8건이
`UNRESOLVED_SERVICE_CALL`**이었고, 전부 같은 모양이었다 — *"Api가 호출하는 `service.fHistoryQry(...)`가
Service에 정의돼 있지 않습니다"*.

**원인**: `generate_skeletons()`가 단순 위임 F 메서드를 D 메서드명 기준으로 **개명**하고 있었다
(`fHistoryQry` → `historyqry`). 이 개명의 원래 이유는 DTO 타입명을 거기서 파생시키려던 것이었는데,
2026-09-05에 시그니처를 `Map<String,Object>`로 통일하면서 그 이유가 사라졌다. 같은 날 P 계층이
LLM 포팅 대상이 되면서(`detect_p_orchestration`) 모델은 당연히 **원본 이름**으로 `service.fHistoryQry(...)`를
쓰는데 Service에는 `historyqry`만 있어, 오케스트레이션을 가진 화면마다 이 충돌이 났다.

**우회가 세 겹 쌓여 있었다**: ① 수리 루프가 그 8건을 LLM으로 다시 고쳤고(LLM 호출을 태우며) ②
L3 하네스는 `m[1:].lower()`로 개명을 되돌려 짝지었으며(2026-09-05 §8.4d의 "개명을 풀어 짝지어
재보니"가 이것이다) ③ 대본은 그 10건을 "검증이 결함을 잡았다"로 설명하고 있었다. 아무도 개명
자체를 의심하지 않았다.

**수정**: 원본 F 메서드명을 그대로 쓴다(`simple_delegations[f_method] = (f_method, ...)`). F 메서드명은
클래스 안에서 유일해 충돌 위험이 없다 — 오히려 D 메서드명 기준 개명이 두 F가 같은 D를 부를 때
충돌했다.
...
| | 수정 전 | 수정 후 |
|---|---|---|
| 정적 검증 BLOCKER | 10건 | **2건** (정답키에 심어둔 결함 2건 그대로) |
| 자가 교정 대상 | 9건 | **1건** |
| 잔여 BLOCKER | 1건 | 1건 (동일 — XML, 사람 이관) |
| 업무 로직(F) 일치 | 48/48 | 48/48 (회귀 없음) |
| 화면 요청(Api) 페이로드 | 9/9 | 9/9 (회귀 없음) |
| LLM 포팅 호출 | 25건 | 25건 (동일) |

**부수 효과 — ToT에 후보 탈락 장면이 처음 생겼다.** 이전에는 수리 대상이 9건이고 후보 두 개가 모두
잔여 0건이라 동점이었다(발표 자료에도 "후보가 탈락하는 장면은 이번 실행에 없다"고 적어뒀다).
수리 대상이 진짜 결함 1건으로 좁혀지자 `[최소수정]` 잔여 1건 · `[계약우선]` 잔여 0건으로 갈려
**검증기가 실제로 하나를 탈락시킨다**. 두 후보 모두 정답키와 같은 `dPLA08701`을 찾았고, 둘을 가른
것은 모델 자기평가가 아니라 splice 후 재검증 수치다.

**재발 방지**: `tests/test_skeleton_naming.py`(신규) — 개명을 되돌리면 실패하는 것까지 확인했다.
`tools/check_demo_script.py`(신규)가 발표 자료의 인용 수치를 실제 로그와 대조한다.

### 조회 질의를 팝업에서 대화 패널로 — 도구 호출 루프(2026-09-12, 사용자 제안)

사용자 제안: *"영향도 질의나 사용법 가이드를 팝업이 아니라 화면 옆 채팅창에서 LLM과 대화하는
형식으로, 검색하듯 조회 기능을 붙이면 어떤가."*

**만들 게 거의 없었다 — 조각이 이미 다 있었다.** `agents/mcp_server.py`의 `_tools()`가 이미
OpenAI 호환에 가까운 스키마(name/description/inputSchema)를 돌려주고, `call_tool(name, args)`가
전송 계층과 분리된 디스패처다. 확인이 필요했던 건 세 가지뿐이었고 전부 실측으로 해소했다.
  1. 사내 게이트웨이가 function calling을 지원하는가 → **지원한다**(실호출로 확인,
     `impact_of_method` + 올바른 인자를 모델이 스스로 골랐다)
  2. `st.chat_input`을 컬럼/사이드바에 둘 수 있는가 → **가능**(Streamlit 1.61.1, AppTest로 확인)
  3. 도구 목록을 두 곳에서 관리하게 되는가 → **아니다**. `openai_tools()`가 MCP의 `_tools()`를
     모양만 바꿔 쓴다 — 목록이 갈라지면 반드시 어긋난다는 걸 이 프로젝트에서 이미 겪었다.

**구현**: `agents/query_agent.py`(신규) — 질문 → 도구 선택 → 조회 실행 → 결과로 답변, 회차 상한 4.
`agents/llm_gateway.chat_with_tools()`(신규, 응답 메시지를 그대로 반환). UI는 `chatui/app.py`의
`_render_query_panel()`로 사이드바에 상주시킨다. 팝업(`_show_impact_dialog`)은 함수명을 정확히
아는 경우 더 빠르므로 **지우지 않고 함께 둔다**.

**원칙을 지킨 지점**
- 노출 도구는 MCP와 **같은 읽기 전용 4종**이다. 변환 실행·저장은 목록에 없어 모델이 고를 수 없다 —
  "완전 자율 탐색형 에이전트를 만들지 않는다"와 MCP 도입 때의 "표면을 갈랐다"는 조건 그대로다.
- 답의 근거는 전부 결정론적 조회 결과다. LLM은 **어느 조회를 부를지 고르는 일**과 **결과를
  문장으로 옮기는 일**만 한다.
- 조회 결과에 없는 식별자가 답변에 나오면 `QUERY_ANSWER_INVENTED_REF`(BLOCKER)로 막는다.
  한글 조사가 붙은 경우(`dPLA99999를`)도 잡는다 — ``를 쓰면 여기서 뚫린다(impact_advisor와 동일).
- **응답 캐시를 타지 않는다.** 캐시 키가 messages만 보므로 DB가 갱신되면 낡은 답을 재생한다.

**실 LLM·실 DB 검증**: "dPLA04702를 고치면 어디가 영향받아?" → `impact_of_method(dPLA04702)`를
스스로 골라 호출자 2건·영향 화면 PLA047·트랜잭션 pPLA04701을 답했고, 요구하지 않은 한계 문구
("정적 분석 기준이라 확정이 아니라 검토 범위")까지 붙였다. "PLA047 트랜잭션 매핑 알려줘" →
`nctrid_map(screen_id=PLA047)` 6건. 두 질문 모두 가드레일 이슈 0건.

**테스트**: `tests/test_query_agent.py` 6건 — 도구 선택·근거 없는 이름 차단·한글 조사 우회 차단·
회차 상한·조회 실패 시 대화 유지·**읽기 전용 4종만 노출**. LLM도 DB도 없이 돈다(주입).

### 조회 패널을 본문 오른쪽으로 + 의도 분기 확장(2026-09-12, 사용자 제안)

**레이아웃**: 사이드바에서 본문 오른쪽 분할로 옮겼다. 본문이 1,127줄이라 `with main_col:`로
감싸려면 전부 들여써야 하고, 그러면 `help=` 문자열과 CSS 블록의 **내용까지 바뀐다**(삼중따옴표
6곳). 재보니 최상위 문장이 10개뿐이었고, **컬럼 컨텍스트를 열고 닫지 않으면** 이후 최상위
요소가 전부 그 안으로 들어간다 — AppTest로 배치를 확인하고(제목·본문은 왼쪽, 부제·예시
버튼·입력창은 오른쪽) 한 줄로 끝냈다. 대량 재들여쓰기를 하지 않았다.

**의도 분기**: 별도 의도 분류기를 만들지 않았다. function calling 루프가 이미 그 일을 하고,
분류 단계를 앞에 붙이면 **틀릴 자리만 하나 늘어난다**. 대신 도구를 3종 더해 고를 선택지를 넓혔다.

| 도구 | 답하는 질문 | 근거 |
|---|---|---|
| `how_to_use` | "어떻게 쓰나", "BLOCKER가 뭔가" | `query_tools.USAGE` 고정 문구 |
| `screen_report` | "이 화면 왜 안 되나", "무엇을 LLM이 옮겼나" | `tracking/conversion-plans/*.json` + 인수인계 문서 |
| `read_screen_source` | "이 함수 무슨 일 하나" | AS-IS 원본 메서드 본문 |

**경로 탐색을 구조로 막았다.** `read_screen_source`는 **변환 계획서가 기록한
`fragments[*].as_is_path`만** 연다 — 호출자가 경로를 넘길 자리가 스펙에 아예 없다(테스트가
이를 검사한다). 임의 경로를 받았다면 `.env`까지 닿았을 자리다. 본문은 6,000자에서 자른다
(`fPLA047QrySelectRev` 본문이 27,426자였다).

**MCP에는 넣지 않았다.** MCP 4종은 외부 클라이언트에 여는 표면이고 거기 노출하는 건
메타데이터다. 소스 본문을 범용 프로토콜 엔드포인트로 흘려보내지 않는다는 게 이 프로젝트의
보안 전제라(CLAUDE.md "소스코드 외부 전송"), 패널 전용으로 뒀다. 도구 정의는 여전히 한 곳에만
있고, `query_agent._tool_specs()`가 두 목록을 합쳐 쓴다.

**실 LLM 검증 — 의도 5종이 전부 맞는 도구로 갔다**

| 질문 | 고른 도구 | 결과 |
|---|---|---|
| 어떻게 쓰나 + BLOCKER가 뭔가 | `how_to_use(파이프라인)` + `how_to_use(판정체계)` | 두 번 호출해 합쳐 답함 |
| PLA047 원본이 컴파일되나 | `screen_report(PLA047)` | "F.java 중괄호 11개 불균형"으로 원인 지목 |
| fPLA047QrySelectRev가 뭘 하나 | `read_screen_source(...)` | dPLA04701 호출 → REV_LIST 반환으로 정확히 설명 |
| dPLA04702 고치면 어디가 영향받나 | `impact_of_method(dPLA04702)` | 호출자 2건·화면 PLA047 |
| **지금 변환해서 저장해줘** | **없음** | "조회만 가능하다"고 거절 — 도구를 부르지 않았다 |

다섯 건 모두 가드레일 이슈 0건. 마지막 건이 중요하다 — 쓰기 도구가 목록에 없으니 **모델이
고를 수가 없고**, 그래서 거절이 프롬프트 설득이 아니라 구조로 보장된다.

**테스트**: `tests/test_query_tools.py` 9건(사용법 문구가 실제 단계·판정과 일치하는지, 경로
파라미터 부재, 없는 화면·없는 메서드 거절, 본문 상한), `tests/test_query_agent.py`에 도구
목록↔실행처 일치 검사 추가. 전체 20건 통과.
