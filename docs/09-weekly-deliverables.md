...
- **기술 목표**: GaiA/Multi-Agent 경험을 코드 변환 도메인으로 확장 — 결정론적 변환기(iBatis→MyBatis, BizUnit→Controller/Service/Store 골격) + LLM 포팅 파이프라인 + 검증기(Validator)를 분리된 모듈로 구현하는 역량 확보
- **비즈니스 목표**: 화면 유형별 대표 파일럿(PLA047 포함 20~30건 목표, 현재 1건 진행)을 통해 화면당 실제 공수 절감률을 실측 데이터로 확보
