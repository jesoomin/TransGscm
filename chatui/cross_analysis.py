...
def analyze_pilot_folder(pilot_root: Path) -> CrossAnalysisResult:
    """pilot/{screen}/ 하위의 TO-BE 산출물을 전부 훑어 화면 간 중복 로직/SQL을 찾는다.

    같은 화면 안에서의 중복은 다루지 않는다(그건 그 화면 자체의 설계 문제) - 여기서는 서로 다른
    화면 사이의 중복만 본다. 실제로 여러 화면이 쌓이기 전까지는 항상 "비교 불가" 노트만 나온다.
    """
    screen_dirs = sorted(p.name for p in pilot_root.iterdir() if p.is_dir()) if pilot_root.exists() else []
    result = CrossAnalysisResult(screens_analyzed=screen_dirs)

    if len(screen_dirs) < 2:
        result.notes.append(
            f"파일럿 화면이 {len(screen_dirs)}개뿐이라 화면 간 중복/영향도 비교를 할 수 없습니다"
    ...
    for screen in screen_dirs:
        screen_dir = pilot_root / screen
        for service_path in list(screen_dir.rglob("*Service.java")) + list(screen_dir.rglob("*Store.java")):
        ...
        for mapper_path in screen_dir.rglob("*Mapper.xml"):
    ...
    if not result.duplicate_groups:
        result.notes.append(f"{len(screen_dirs)}개 화면({', '.join(screen_dirs)})을 비교했지만 화면 간 중복(동일 로직/SQL)은 발견되지 않았습니다.")
