...
- 생성된 파일: 3개 (Pla048Mapper.xml, Pla048Service.java, Pla048Store.java)
...
- **ORIGINAL_BUG** (메서드 `fAuthCheck`, 46행) — 46행: 원본 버그(포팅 시 보존, 임의 수정 안 함) - 원본은 IRecordSet.getRecordCount()에 전적으로 의존하므로, 프레임워크 제거 후 AUTH_LIST 반환 타입이 불명확한 경우의 정확한 건수 판단 기준이 명시되어 있지 않다.
  - 발견: 품질·취약점 스캔(Pla048Service.java)
...
- `fAuthCheck`
- `fPLA048QrySelectMainList`
- `fPLA048QrySelectDetail`
