...
- **RESPONSE_MESSAGE_CONVENTION_UNDEFINED** (메서드 `pPLA04901`) — pPLA04901가 반환하는 결과 메시지 코드(I0016, W0024)를 담을 TO-BE 응답 규약이 아직 확정되지 않았습니다(docs/09-common-response-convention.md 열린 질문 2번). 규약 없이 임의의 키를 만들지 않았으므로 이 메시지는 현재 TO-BE 응답에 실리지 않습니다 - 사람이 규약을 확정해야 해소됩니다.
  - 발견: 골격 생성(skeleton_gen)
- **RESPONSE_MESSAGE_CONVENTION_UNDEFINED** (메서드 `pPLA04902`) — pPLA04902가 반환하는 결과 메시지 코드(I0016, W0024)를 담을 TO-BE 응답 규약이 아직 확정되지 않았습니다(docs/09-common-response-convention.md 열린 질문 2번). 규약 없이 임의의 키를 만들지 않았으므로 이 메시지는 현재 TO-BE 응답에 실리지 않습니다 - 사람이 규약을 확정해야 해소됩니다.
  - 발견: 골격 생성(skeleton_gen)
...
- **P_ORCHESTRATION_PORT_REQUIRED** (메서드 `pPLA04901`) — pPLA04901는 순수 위임이 아닙니다(F 메서드를 2개 호출(fAuthCheck, fPLA049QrySelectMainList); 권한 게이트(AUTH_YN 확인 후 조기 반환); 결과 메시지 코드 I0016, W0024; 레코드셋 선별 반환(MAIN_LIST)) - Api를 위임 한 줄로 생성하지 않고 LLM 포팅 대상으로 남겼습니다.
  - 발견: 골격 생성(skeleton_gen)
- **P_ORCHESTRATION_PORT_REQUIRED** (메서드 `pPLA04902`) — pPLA04902는 순수 위임이 아닙니다(결과 메시지 코드 I0016, W0024; 레코드셋 선별 반환(DETAIL_LIST)) - Api를 위임 한 줄로 생성하지 않고 LLM 포팅 대상으로 남겼습니다.
  - 발견: 골격 생성(skeleton_gen)
- **P_ORCHESTRATION_PORT_REQUIRED** (메서드 `pPLA04903`) — pPLA04903는 순수 위임이 아닙니다(결과 메시지 코드 I0016; 레코드셋 선별 반환(HIST_LIST)) - Api를 위임 한 줄로 생성하지 않고 LLM 포팅 대상으로 남겼습니다.
  - 발견: 골격 생성(skeleton_gen)
- **P_ORCHESTRATION_PORT_REQUIRED** (메서드 `pPLA04904`) — pPLA04904는 순수 위임이 아닙니다(결과 메시지 코드 I0016; 레코드셋 선별 반환(EXCEL_LIST)) - Api를 위임 한 줄로 생성하지 않고 LLM 포팅 대상으로 남겼습니다.
  - 발견: 골격 생성(skeleton_gen)
...
- **DTO_FIELD_EXTRACT_INCOMPLETE** (메서드 `pPLA04901`) — pPLA04901: fAuthCheck에서 개별 getField 호출을 찾지 못했습니다 (getFieldMap()으로 통째로 넘기는 구조일 수 있음) - 요청 필드를 수동으로 확인하세요.
...
- **DTO_FIELD_EXTRACT_INCOMPLETE** (메서드 `pPLA04903`) — pPLA04903: fHistoryQry에서 개별 getField 호출을 찾지 못했습니다 (getFieldMap()으로 통째로 넘기는 구조일 수 있음) - 요청 필드를 수동으로 확인하세요.
  - 발견: DTO 생성(skeleton_gen)
  - 조치: 요청/응답 필드를 자동으로 못 뽑아 DTO에 TODO가 남아 있습니다(F가 getFieldMap()으로 통째로 넘기는 구조 등). 원본에서 실제 사용 필드를 확인해 채우세요 - 추측으로 채우지 마세요.
- **DTO_FIELD_EXTRACT_INCOMPLETE** (메서드 `pPLA04904`) — pPLA04904: fExcelDownQry에서 개별 getField 호출을 찾지 못했습니다 (getFieldMap()으로 통째로 넘기는 구조일 수 있음) - 요청 필드를 수동으로 확인하세요.
  - 발견: DTO 생성(skeleton_gen)
  - 조치: 요청/응답 필드를 자동으로 못 뽑아 DTO에 TODO가 남아 있습니다(F가 getFieldMap()으로 통째로 넘기는 구조 등). 원본에서 실제 사용 필드를 확인해 채우세요 - 추측으로 채우지 마세요.
...
- **COMMON_STATEMENT_EXTRACTED** — <select id="S902">는 화면 간 공통 SQL로 확정돼 있어 이 Mapper에서 제외했습니다 - 공통 Mapper 한 곳에서 관리합니다(config/common-methods.json).
  - 발견: Mapper 변환(converters)
- **COMMON_STATEMENT_EXTRACTED** — <select id="S904">는 화면 간 공통 SQL로 확정돼 있어 이 Mapper에서 제외했습니다 - 공통 Mapper 한 곳에서 관리합니다(config/common-methods.json).
  - 발견: Mapper 변환(converters)
...
- **ORIGINAL_BUG** (메서드 `fAuthCheck`, 41행) — 41행: 원본 버그(포팅 시 보존, 임의 수정 안 함) - 원본은 dAuthCheck(...).getRecordSet("AUTH_LIST") 호출을 전제하지만, 포팅 대상 반환 타입 명세가 없어 AUTH_LIST 추출 불가 시 그대로 null 처리함.
...
| 파일 | TO-BE 경로 | 변환 방식 |
|---|---|---|
| Pla049Api.java | `gscm/src/main/java/com/skhynix/gscm/r/pm/pla/Controller/Pla049Api.java` | RULE_BASED |
| Pla049Dto.java | `gscm/src/main/java/com/skhynix/gscm/r/pm/pla/dto/Pla049Dto.java` | RULE_BASED |
