...
*/
public class FPLA040 extends gscm.fwk.base.ProcessUnit{
	public FPLA040(){
		super();
	}
	...
	*/
	public IDataSet fPLA040QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA040 fu = (DPLA040) lookupFunctionUnit(DPLA040.class);
			...
			rdPivot = du.dPLA04002(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04005(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04004(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04003(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA040QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA040 du = (DPLA040) lookupDataUnit(DPLA040.class);
			IRecordSet rs = du.dPLA04001(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA040QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA040 du = (DPLA040) lookupDataUnit(DPLA040.class);
			IRecordSet rs = du.dPLA04006(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
