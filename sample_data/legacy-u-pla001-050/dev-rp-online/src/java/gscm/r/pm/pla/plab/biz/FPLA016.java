...
*/
public class FPLA016 extends gscm.fwk.base.ProcessUnit{
	public FPLA016(){
		super();
	}
	...
	*/
	public IDataSet fPLA016QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA016 fu = (DPLA016) lookupFunctionUnit(DPLA016.class);
			...
			rdPivot = du.dPLA01602(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01605(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01604(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01603(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA016QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA016 du = (DPLA016) lookupDataUnit(DPLA016.class);
			IRecordSet rs = du.dPLA01601(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA016QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA016 du = (DPLA016) lookupDataUnit(DPLA016.class);
			IRecordSet rs = du.dPLA01606(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
