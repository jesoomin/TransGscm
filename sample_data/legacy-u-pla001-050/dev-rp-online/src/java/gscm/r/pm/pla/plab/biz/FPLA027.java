...
*/
public class FPLA027 extends gscm.fwk.base.ProcessUnit{
	public FPLA027(){
		super();
	}
	...
	*/
	public IDataSet fPLA027QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA027 fu = (DPLA027) lookupFunctionUnit(DPLA027.class);
			...
			rdPivot = du.dPLA02702(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02705(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02704(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02703(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA027QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA027 du = (DPLA027) lookupDataUnit(DPLA027.class);
			IRecordSet rs = du.dPLA02701(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA027QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA027 du = (DPLA027) lookupDataUnit(DPLA027.class);
			IRecordSet rs = du.dPLA02706(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
