...
*/
public class FPLA024 extends gscm.fwk.base.ProcessUnit{
	public FPLA024(){
		super();
	}
	...
	*/
	public IDataSet fPLA024QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA024 fu = (DPLA024) lookupFunctionUnit(DPLA024.class);
			...
			rdPivot = du.dPLA02402(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02405(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02404(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02403(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA024QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA024 du = (DPLA024) lookupDataUnit(DPLA024.class);
			IRecordSet rs = du.dPLA02401(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA024QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA024 du = (DPLA024) lookupDataUnit(DPLA024.class);
			IRecordSet rs = du.dPLA02406(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
