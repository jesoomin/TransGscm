...
*/
public class FPLA031 extends gscm.fwk.base.ProcessUnit{
	public FPLA031(){
		super();
	}
	...
	*/
	public IDataSet fPLA031QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA031 fu = (DPLA031) lookupFunctionUnit(DPLA031.class);
			...
			rdPivot = du.dPLA03102(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03105(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03104(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03103(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA031QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA031 du = (DPLA031) lookupDataUnit(DPLA031.class);
			IRecordSet rs = du.dPLA03101(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA031QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA031 du = (DPLA031) lookupDataUnit(DPLA031.class);
			IRecordSet rs = du.dPLA03106(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
