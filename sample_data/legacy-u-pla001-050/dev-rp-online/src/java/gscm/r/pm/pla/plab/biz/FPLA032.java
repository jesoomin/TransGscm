...
*/
public class FPLA032 extends gscm.fwk.base.ProcessUnit{
	public FPLA032(){
		super();
	}
	...
	*/
	public IDataSet fPLA032QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA032 fu = (DPLA032) lookupFunctionUnit(DPLA032.class);
			...
			rdPivot = du.dPLA03202(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03205(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03204(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03203(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA032QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA032 du = (DPLA032) lookupDataUnit(DPLA032.class);
			IRecordSet rs = du.dPLA03201(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA032QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA032 du = (DPLA032) lookupDataUnit(DPLA032.class);
			IRecordSet rs = du.dPLA03206(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
