...
*/
public class FPLA002 extends gscm.fwk.base.ProcessUnit{
	public FPLA002(){
		super();
	}
	...
	*/
	public IDataSet fPLA002QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA002 fu = (DPLA002) lookupFunctionUnit(DPLA002.class);
			...
			rdPivot = du.dPLA00202(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00205(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00204(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00203(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA002QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA002 du = (DPLA002) lookupDataUnit(DPLA002.class);
			IRecordSet rs = du.dPLA00201(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA002QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA002 du = (DPLA002) lookupDataUnit(DPLA002.class);
			IRecordSet rs = du.dPLA00206(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
