...
*/
public class FPLA001 extends gscm.fwk.base.ProcessUnit{
	public FPLA001(){
		super();
	}
	...
	*/
	public IDataSet fPLA001QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA001 fu = (DPLA001) lookupFunctionUnit(DPLA001.class);
			...
			rdPivot = du.dPLA00102(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00105(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00104(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00103(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA001QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA001 du = (DPLA001) lookupDataUnit(DPLA001.class);
			IRecordSet rs = du.dPLA00101(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA001QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA001 du = (DPLA001) lookupDataUnit(DPLA001.class);
			IRecordSet rs = du.dPLA00106(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
