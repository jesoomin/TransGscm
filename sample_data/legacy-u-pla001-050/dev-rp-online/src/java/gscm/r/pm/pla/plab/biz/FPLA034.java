...
*/
public class FPLA034 extends gscm.fwk.base.ProcessUnit{
	public FPLA034(){
		super();
	}
	...
	*/
	public IDataSet fPLA034QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA034 fu = (DPLA034) lookupFunctionUnit(DPLA034.class);
			...
			rdPivot = du.dPLA03402(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03405(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03404(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03403(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA034QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA034 du = (DPLA034) lookupDataUnit(DPLA034.class);
			IRecordSet rs = du.dPLA03401(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA034QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA034 du = (DPLA034) lookupDataUnit(DPLA034.class);
			IRecordSet rs = du.dPLA03406(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
