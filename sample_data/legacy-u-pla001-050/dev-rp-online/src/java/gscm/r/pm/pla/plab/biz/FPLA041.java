...
*/
public class FPLA041 extends gscm.fwk.base.ProcessUnit{
	public FPLA041(){
		super();
	}
	...
	*/
	public IDataSet fPLA041QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA041 fu = (DPLA041) lookupFunctionUnit(DPLA041.class);
			...
			rdPivot = du.dPLA04102(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04105(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04104(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04103(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA041QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA041 du = (DPLA041) lookupDataUnit(DPLA041.class);
			IRecordSet rs = du.dPLA04101(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA041QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA041 du = (DPLA041) lookupDataUnit(DPLA041.class);
			IRecordSet rs = du.dPLA04106(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
