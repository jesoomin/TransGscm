...
*/
public class FPLA021 extends gscm.fwk.base.ProcessUnit{
	public FPLA021(){
		super();
	}
	...
	*/
	public IDataSet fPLA021QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA021 fu = (DPLA021) lookupFunctionUnit(DPLA021.class);
			...
			rdPivot = du.dPLA02102(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02105(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02104(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02103(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA021QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA021 du = (DPLA021) lookupDataUnit(DPLA021.class);
			IRecordSet rs = du.dPLA02101(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA021QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA021 du = (DPLA021) lookupDataUnit(DPLA021.class);
			IRecordSet rs = du.dPLA02106(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
