...
*/
public class FPLA037 extends gscm.fwk.base.ProcessUnit{
	public FPLA037(){
		super();
	}
	...
	*/
	public IDataSet fPLA037QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA037 fu = (DPLA037) lookupFunctionUnit(DPLA037.class);
			...
			rdPivot = du.dPLA03702(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03705(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03704(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03703(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA037QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA037 du = (DPLA037) lookupDataUnit(DPLA037.class);
			IRecordSet rs = du.dPLA03701(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA037QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA037 du = (DPLA037) lookupDataUnit(DPLA037.class);
			IRecordSet rs = du.dPLA03706(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
