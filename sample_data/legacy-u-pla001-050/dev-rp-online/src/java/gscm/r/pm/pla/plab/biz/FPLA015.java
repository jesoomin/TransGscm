...
*/
public class FPLA015 extends gscm.fwk.base.ProcessUnit{
	public FPLA015(){
		super();
	}
	...
	*/
	public IDataSet fPLA015QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA015 fu = (DPLA015) lookupFunctionUnit(DPLA015.class);
			...
			rdPivot = du.dPLA01502(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01505(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01504(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01503(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA015QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA015 du = (DPLA015) lookupDataUnit(DPLA015.class);
			IRecordSet rs = du.dPLA01501(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA015QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA015 du = (DPLA015) lookupDataUnit(DPLA015.class);
			IRecordSet rs = du.dPLA01506(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
