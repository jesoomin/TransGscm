...
*/
public class FPLA011 extends gscm.fwk.base.ProcessUnit{
	public FPLA011(){
		super();
	}
	...
	*/
	public IDataSet fPLA011QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA011 fu = (DPLA011) lookupFunctionUnit(DPLA011.class);
			...
			rdPivot = du.dPLA01102(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01105(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01104(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01103(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA011QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA011 du = (DPLA011) lookupDataUnit(DPLA011.class);
			IRecordSet rs = du.dPLA01101(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA011QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA011 du = (DPLA011) lookupDataUnit(DPLA011.class);
			IRecordSet rs = du.dPLA01106(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
