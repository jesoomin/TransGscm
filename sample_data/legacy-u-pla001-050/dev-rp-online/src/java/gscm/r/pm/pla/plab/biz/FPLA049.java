...
*/
public class FPLA049 extends gscm.fwk.base.ProcessUnit{
	public FPLA049(){
		super();
	}
	...
	*/
	public IDataSet fPLA049QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA049 fu = (DPLA049) lookupFunctionUnit(DPLA049.class);
			...
			rdPivot = du.dPLA04902(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04905(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04904(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04903(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA049QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA049 du = (DPLA049) lookupDataUnit(DPLA049.class);
			IRecordSet rs = du.dPLA04901(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA049QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA049 du = (DPLA049) lookupDataUnit(DPLA049.class);
			IRecordSet rs = du.dPLA04906(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
