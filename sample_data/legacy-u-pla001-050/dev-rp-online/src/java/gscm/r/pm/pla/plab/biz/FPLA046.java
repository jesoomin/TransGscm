...
*/
public class FPLA046 extends gscm.fwk.base.ProcessUnit{
	public FPLA046(){
		super();
	}
	...
	*/
	public IDataSet fPLA046QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA046 fu = (DPLA046) lookupFunctionUnit(DPLA046.class);
			...
			rdPivot = du.dPLA04602(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04605(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04604(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04603(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA046QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA046 du = (DPLA046) lookupDataUnit(DPLA046.class);
			IRecordSet rs = du.dPLA04601(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA046QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA046 du = (DPLA046) lookupDataUnit(DPLA046.class);
			IRecordSet rs = du.dPLA04606(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
