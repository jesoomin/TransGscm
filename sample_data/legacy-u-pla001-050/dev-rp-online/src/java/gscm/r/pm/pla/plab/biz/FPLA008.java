...
*/
public class FPLA008 extends gscm.fwk.base.ProcessUnit{
	public FPLA008(){
		super();
	}
	...
	*/
	public IDataSet fPLA008QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA008 fu = (DPLA008) lookupFunctionUnit(DPLA008.class);
			...
			rdPivot = du.dPLA00802(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00805(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00804(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00803(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA008QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA008 du = (DPLA008) lookupDataUnit(DPLA008.class);
			IRecordSet rs = du.dPLA00801(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA008QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA008 du = (DPLA008) lookupDataUnit(DPLA008.class);
			IRecordSet rs = du.dPLA00806(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
