...
*/
public class FPLA005 extends gscm.fwk.base.ProcessUnit{
	public FPLA005(){
		super();
	}
	...
	*/
	public IDataSet fPLA005QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA005 fu = (DPLA005) lookupFunctionUnit(DPLA005.class);
			...
			rdPivot = du.dPLA00502(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00505(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00504(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00503(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA005QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA005 du = (DPLA005) lookupDataUnit(DPLA005.class);
			IRecordSet rs = du.dPLA00501(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA005QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA005 du = (DPLA005) lookupDataUnit(DPLA005.class);
			IRecordSet rs = du.dPLA00506(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
