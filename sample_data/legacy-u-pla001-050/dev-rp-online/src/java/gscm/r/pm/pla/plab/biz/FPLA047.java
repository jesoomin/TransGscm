...
*/
public class FPLA047 extends gscm.fwk.base.ProcessUnit{
	public FPLA047(){
		super();
	}
	...
	*/
	public IDataSet fPLA047QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA047 fu = (DPLA047) lookupFunctionUnit(DPLA047.class);
			...
			rdPivot = du.dPLA04702(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04705(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04704(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04703(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	...
	*/
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA047QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA047 du = (DPLA047) lookupDataUnit(DPLA047.class);
			IRecordSet rs = du.dPLA04706(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
