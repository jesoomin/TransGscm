...
*/
public class FPLA003 extends gscm.fwk.base.ProcessUnit{
	public FPLA003(){
		super();
	}
	...
	*/
	public IDataSet fPLA003QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA003 fu = (DPLA003) lookupFunctionUnit(DPLA003.class);
			...
			rdPivot = du.dPLA00302(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00305(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00304(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00303(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA003QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA003 du = (DPLA003) lookupDataUnit(DPLA003.class);
			IRecordSet rs = du.dPLA00301(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA003QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA003 du = (DPLA003) lookupDataUnit(DPLA003.class);
			IRecordSet rs = du.dPLA00306(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
