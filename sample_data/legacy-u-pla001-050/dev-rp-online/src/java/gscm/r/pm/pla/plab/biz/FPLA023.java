...
*/
public class FPLA023 extends gscm.fwk.base.ProcessUnit{
	public FPLA023(){
		super();
	}
	...
	*/
	public IDataSet fPLA023QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA023 fu = (DPLA023) lookupFunctionUnit(DPLA023.class);
			...
			rdPivot = du.dPLA02302(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02305(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02304(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02303(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA023QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA023 du = (DPLA023) lookupDataUnit(DPLA023.class);
			IRecordSet rs = du.dPLA02301(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA023QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA023 du = (DPLA023) lookupDataUnit(DPLA023.class);
			IRecordSet rs = du.dPLA02306(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
