...
*/
public class FPLA042 extends gscm.fwk.base.ProcessUnit{
	public FPLA042(){
		super();
	}
	...
	*/
	public IDataSet fPLA042QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA042 fu = (DPLA042) lookupFunctionUnit(DPLA042.class);
			...
			rdPivot = du.dPLA04202(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04205(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04204(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04203(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA042QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA042 du = (DPLA042) lookupDataUnit(DPLA042.class);
			IRecordSet rs = du.dPLA04201(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA042QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA042 du = (DPLA042) lookupDataUnit(DPLA042.class);
			IRecordSet rs = du.dPLA04206(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
