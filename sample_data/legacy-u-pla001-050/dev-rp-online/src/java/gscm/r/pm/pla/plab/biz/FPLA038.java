...
*/
public class FPLA038 extends gscm.fwk.base.ProcessUnit{
	public FPLA038(){
		super();
	}
	...
	*/
	public IDataSet fPLA038QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA038 fu = (DPLA038) lookupFunctionUnit(DPLA038.class);
			...
			rdPivot = du.dPLA03802(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03805(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03804(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03803(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA038QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA038 du = (DPLA038) lookupDataUnit(DPLA038.class);
			IRecordSet rs = du.dPLA03801(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA038QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA038 du = (DPLA038) lookupDataUnit(DPLA038.class);
			IRecordSet rs = du.dPLA03806(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
