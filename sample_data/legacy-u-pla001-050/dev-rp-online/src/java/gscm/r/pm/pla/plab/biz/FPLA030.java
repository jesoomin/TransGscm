...
*/
public class FPLA030 extends gscm.fwk.base.ProcessUnit{
	public FPLA030(){
		super();
	}
	...
	*/
	public IDataSet fPLA030QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA030 fu = (DPLA030) lookupFunctionUnit(DPLA030.class);
			...
			rdPivot = du.dPLA03002(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03005(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03004(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03003(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA030QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA030 du = (DPLA030) lookupDataUnit(DPLA030.class);
			IRecordSet rs = du.dPLA03001(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA030QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA030 du = (DPLA030) lookupDataUnit(DPLA030.class);
			IRecordSet rs = du.dPLA03006(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
