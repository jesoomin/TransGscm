...
*/
public class FPLA013 extends gscm.fwk.base.ProcessUnit{
	public FPLA013(){
		super();
	}
	...
	*/
	public IDataSet fPLA013QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA013 fu = (DPLA013) lookupFunctionUnit(DPLA013.class);
			...
			rdPivot = du.dPLA01302(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01305(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01304(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01303(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA013QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA013 du = (DPLA013) lookupDataUnit(DPLA013.class);
			IRecordSet rs = du.dPLA01301(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA013QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA013 du = (DPLA013) lookupDataUnit(DPLA013.class);
			IRecordSet rs = du.dPLA01306(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
