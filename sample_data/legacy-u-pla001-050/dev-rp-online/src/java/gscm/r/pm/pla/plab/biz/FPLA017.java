...
*/
public class FPLA017 extends gscm.fwk.base.ProcessUnit{
	public FPLA017(){
		super();
	}
	...
	*/
	public IDataSet fPLA017QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA017 fu = (DPLA017) lookupFunctionUnit(DPLA017.class);
			...
			rdPivot = du.dPLA01702(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01705(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01704(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01703(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA017QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA017 du = (DPLA017) lookupDataUnit(DPLA017.class);
			IRecordSet rs = du.dPLA01701(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA017QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA017 du = (DPLA017) lookupDataUnit(DPLA017.class);
			IRecordSet rs = du.dPLA01706(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
