...
*/
public class FPLA019 extends gscm.fwk.base.ProcessUnit{
	public FPLA019(){
		super();
	}
	...
	*/
	public IDataSet fPLA019QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA019 fu = (DPLA019) lookupFunctionUnit(DPLA019.class);
			...
			rdPivot = du.dPLA01902(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01905(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01904(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01903(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA019QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA019 du = (DPLA019) lookupDataUnit(DPLA019.class);
			IRecordSet rs = du.dPLA01901(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA019QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA019 du = (DPLA019) lookupDataUnit(DPLA019.class);
			IRecordSet rs = du.dPLA01906(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
