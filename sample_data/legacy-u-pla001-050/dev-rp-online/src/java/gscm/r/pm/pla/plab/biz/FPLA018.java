...
*/
public class FPLA018 extends gscm.fwk.base.ProcessUnit{
	public FPLA018(){
		super();
	}
	...
	*/
	public IDataSet fPLA018QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA018 fu = (DPLA018) lookupFunctionUnit(DPLA018.class);
			...
			rdPivot = du.dPLA01802(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01805(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01804(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01803(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA018QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA018 du = (DPLA018) lookupDataUnit(DPLA018.class);
			IRecordSet rs = du.dPLA01801(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA018QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA018 du = (DPLA018) lookupDataUnit(DPLA018.class);
			IRecordSet rs = du.dPLA01806(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
