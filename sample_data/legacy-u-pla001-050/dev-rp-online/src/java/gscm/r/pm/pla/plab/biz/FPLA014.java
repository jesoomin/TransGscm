...
*/
public class FPLA014 extends gscm.fwk.base.ProcessUnit{
	public FPLA014(){
		super();
	}
	...
	*/
	public IDataSet fPLA014QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA014 fu = (DPLA014) lookupFunctionUnit(DPLA014.class);
			...
			rdPivot = du.dPLA01402(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01405(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01404(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01403(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA014QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA014 du = (DPLA014) lookupDataUnit(DPLA014.class);
			IRecordSet rs = du.dPLA01401(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA014QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA014 du = (DPLA014) lookupDataUnit(DPLA014.class);
			IRecordSet rs = du.dPLA01406(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
