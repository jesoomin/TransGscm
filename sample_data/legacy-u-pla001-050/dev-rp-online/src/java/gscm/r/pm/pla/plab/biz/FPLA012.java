...
*/
public class FPLA012 extends gscm.fwk.base.ProcessUnit{
	public FPLA012(){
		super();
	}
	...
	*/
	public IDataSet fPLA012QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA012 fu = (DPLA012) lookupFunctionUnit(DPLA012.class);
			...
			rdPivot = du.dPLA01202(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01205(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01204(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01203(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA012QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA012 du = (DPLA012) lookupDataUnit(DPLA012.class);
			IRecordSet rs = du.dPLA01201(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA012QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA012 du = (DPLA012) lookupDataUnit(DPLA012.class);
			IRecordSet rs = du.dPLA01206(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
