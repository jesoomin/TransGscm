...
*/
public class FPLA033 extends gscm.fwk.base.ProcessUnit{
	public FPLA033(){
		super();
	}
	...
	*/
	public IDataSet fPLA033QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA033 fu = (DPLA033) lookupFunctionUnit(DPLA033.class);
			...
			rdPivot = du.dPLA03302(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03305(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03304(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03303(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA033QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA033 du = (DPLA033) lookupDataUnit(DPLA033.class);
			IRecordSet rs = du.dPLA03301(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA033QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA033 du = (DPLA033) lookupDataUnit(DPLA033.class);
			IRecordSet rs = du.dPLA03306(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
