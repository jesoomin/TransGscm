...
*/
public class FPLA050 extends gscm.fwk.base.ProcessUnit{
	public FPLA050(){
		super();
	}
	...
	*/
	public IDataSet fPLA050QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA050 fu = (DPLA050) lookupFunctionUnit(DPLA050.class);
			...
			rdPivot = du.dPLA05002(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA05005(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA05004(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA05003(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA050QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA050 du = (DPLA050) lookupDataUnit(DPLA050.class);
			IRecordSet rs = du.dPLA05001(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA050QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA050 du = (DPLA050) lookupDataUnit(DPLA050.class);
			IRecordSet rs = du.dPLA05006(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
