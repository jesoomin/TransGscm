...
*/
public class FPLA035 extends gscm.fwk.base.ProcessUnit{
	public FPLA035(){
		super();
	}
	...
	*/
	public IDataSet fPLA035QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA035 fu = (DPLA035) lookupFunctionUnit(DPLA035.class);
			...
			rdPivot = du.dPLA03502(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03505(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03504(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03503(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA035QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA035 du = (DPLA035) lookupDataUnit(DPLA035.class);
			IRecordSet rs = du.dPLA03501(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA035QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA035 du = (DPLA035) lookupDataUnit(DPLA035.class);
			IRecordSet rs = du.dPLA03506(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
