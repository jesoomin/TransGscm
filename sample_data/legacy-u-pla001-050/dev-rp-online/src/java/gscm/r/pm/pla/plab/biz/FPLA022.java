...
*/
public class FPLA022 extends gscm.fwk.base.ProcessUnit{
	public FPLA022(){
		super();
	}
	...
	*/
	public IDataSet fPLA022QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA022 fu = (DPLA022) lookupFunctionUnit(DPLA022.class);
			...
			rdPivot = du.dPLA02202(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02205(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02204(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02203(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA022QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA022 du = (DPLA022) lookupDataUnit(DPLA022.class);
			IRecordSet rs = du.dPLA02201(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA022QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA022 du = (DPLA022) lookupDataUnit(DPLA022.class);
			IRecordSet rs = du.dPLA02206(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
