...
*/
public class FPLA036 extends gscm.fwk.base.ProcessUnit{
	public FPLA036(){
		super();
	}
	...
	*/
	public IDataSet fPLA036QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA036 fu = (DPLA036) lookupFunctionUnit(DPLA036.class);
			...
			rdPivot = du.dPLA03602(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA03605(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA03604(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA03603(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA036QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA036 du = (DPLA036) lookupDataUnit(DPLA036.class);
			IRecordSet rs = du.dPLA03601(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA036QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA036 du = (DPLA036) lookupDataUnit(DPLA036.class);
			IRecordSet rs = du.dPLA03606(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
