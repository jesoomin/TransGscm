...
*/
public class FPLA007 extends gscm.fwk.base.ProcessUnit{
	public FPLA007(){
		super();
	}
	...
	*/
	public IDataSet fPLA007QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA007 fu = (DPLA007) lookupFunctionUnit(DPLA007.class);
			...
			rdPivot = du.dPLA00702(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00705(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00704(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00703(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA007QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA007 du = (DPLA007) lookupDataUnit(DPLA007.class);
			IRecordSet rs = du.dPLA00701(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA007QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA007 du = (DPLA007) lookupDataUnit(DPLA007.class);
			IRecordSet rs = du.dPLA00706(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
