...
*/
public class FPLA029 extends gscm.fwk.base.ProcessUnit{
	public FPLA029(){
		super();
	}
	...
	*/
	public IDataSet fPLA029QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA029 fu = (DPLA029) lookupFunctionUnit(DPLA029.class);
			...
			rdPivot = du.dPLA02902(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02905(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02904(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02903(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA029QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA029 du = (DPLA029) lookupDataUnit(DPLA029.class);
			IRecordSet rs = du.dPLA02901(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA029QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA029 du = (DPLA029) lookupDataUnit(DPLA029.class);
			IRecordSet rs = du.dPLA02906(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
