...
*/
public class FPLA010 extends gscm.fwk.base.ProcessUnit{
	public FPLA010(){
		super();
	}
	...
	*/
	public IDataSet fPLA010QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA010 fu = (DPLA010) lookupFunctionUnit(DPLA010.class);
			...
			rdPivot = du.dPLA01002(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA01005(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA01004(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA01003(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA010QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA010 du = (DPLA010) lookupDataUnit(DPLA010.class);
			IRecordSet rs = du.dPLA01001(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA010QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA010 du = (DPLA010) lookupDataUnit(DPLA010.class);
			IRecordSet rs = du.dPLA01006(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
