...
*/
public class FPLA025 extends gscm.fwk.base.ProcessUnit{
	public FPLA025(){
		super();
	}
	...
	*/
	public IDataSet fPLA025QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA025 fu = (DPLA025) lookupFunctionUnit(DPLA025.class);
			...
			rdPivot = du.dPLA02502(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02505(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02504(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02503(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA025QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA025 du = (DPLA025) lookupDataUnit(DPLA025.class);
			IRecordSet rs = du.dPLA02501(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA025QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA025 du = (DPLA025) lookupDataUnit(DPLA025.class);
			IRecordSet rs = du.dPLA02506(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
