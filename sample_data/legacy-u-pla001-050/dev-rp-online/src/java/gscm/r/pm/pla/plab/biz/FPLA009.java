...
*/
public class FPLA009 extends gscm.fwk.base.ProcessUnit{
	public FPLA009(){
		super();
	}
	...
	*/
	public IDataSet fPLA009QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA009 fu = (DPLA009) lookupFunctionUnit(DPLA009.class);
			...
			rdPivot = du.dPLA00902(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00905(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00904(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00903(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA009QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA009 du = (DPLA009) lookupDataUnit(DPLA009.class);
			IRecordSet rs = du.dPLA00901(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA009QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA009 du = (DPLA009) lookupDataUnit(DPLA009.class);
			IRecordSet rs = du.dPLA00906(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
