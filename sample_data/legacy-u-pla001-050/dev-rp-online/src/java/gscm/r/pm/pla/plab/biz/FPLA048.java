...
*/
public class FPLA048 extends gscm.fwk.base.ProcessUnit{
	public FPLA048(){
		super();
	}
	...
	*/
	public IDataSet fPLA048QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA048 fu = (DPLA048) lookupFunctionUnit(DPLA048.class);
			...
			rdPivot = du.dPLA04802(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04805(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04804(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04803(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA048QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA048 du = (DPLA048) lookupDataUnit(DPLA048.class);
			IRecordSet rs = du.dPLA04801(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA048QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA048 du = (DPLA048) lookupDataUnit(DPLA048.class);
			IRecordSet rs = du.dPLA04806(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
