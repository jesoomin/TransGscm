...
*/
public class FPLA020 extends gscm.fwk.base.ProcessUnit{
	public FPLA020(){
		super();
	}
	...
	*/
	public IDataSet fPLA020QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA020 fu = (DPLA020) lookupFunctionUnit(DPLA020.class);
			...
			rdPivot = du.dPLA02002(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02005(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02004(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02003(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA020QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA020 du = (DPLA020) lookupDataUnit(DPLA020.class);
			IRecordSet rs = du.dPLA02001(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA020QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA020 du = (DPLA020) lookupDataUnit(DPLA020.class);
			IRecordSet rs = du.dPLA02006(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
