...
*/
public class FPLA045 extends gscm.fwk.base.ProcessUnit{
	public FPLA045(){
		super();
	}
	...
	*/
	public IDataSet fPLA045QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA045 fu = (DPLA045) lookupFunctionUnit(DPLA045.class);
			...
			rdPivot = du.dPLA04502(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04505(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04504(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04503(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA045QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA045 du = (DPLA045) lookupDataUnit(DPLA045.class);
			IRecordSet rs = du.dPLA04501(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA045QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA045 du = (DPLA045) lookupDataUnit(DPLA045.class);
			IRecordSet rs = du.dPLA04506(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
