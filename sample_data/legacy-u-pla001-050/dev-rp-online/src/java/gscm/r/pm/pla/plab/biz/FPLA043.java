...
*/
public class FPLA043 extends gscm.fwk.base.ProcessUnit{
	public FPLA043(){
		super();
	}
	...
	*/
	public IDataSet fPLA043QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA043 fu = (DPLA043) lookupFunctionUnit(DPLA043.class);
			...
			rdPivot = du.dPLA04302(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04305(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04304(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04303(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA043QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA043 du = (DPLA043) lookupDataUnit(DPLA043.class);
			IRecordSet rs = du.dPLA04301(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA043QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA043 du = (DPLA043) lookupDataUnit(DPLA043.class);
			IRecordSet rs = du.dPLA04306(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
