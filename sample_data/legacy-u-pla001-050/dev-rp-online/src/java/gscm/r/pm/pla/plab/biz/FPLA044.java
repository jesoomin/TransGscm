...
*/
public class FPLA044 extends gscm.fwk.base.ProcessUnit{
	public FPLA044(){
		super();
	}
	...
	*/
	public IDataSet fPLA044QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA044 fu = (DPLA044) lookupFunctionUnit(DPLA044.class);
			...
			rdPivot = du.dPLA04402(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA04405(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA04404(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA04403(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA044QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA044 du = (DPLA044) lookupDataUnit(DPLA044.class);
			IRecordSet rs = du.dPLA04401(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA044QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA044 du = (DPLA044) lookupDataUnit(DPLA044.class);
			IRecordSet rs = du.dPLA04406(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
