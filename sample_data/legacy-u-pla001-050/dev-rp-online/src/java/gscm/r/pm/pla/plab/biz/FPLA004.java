...
*/
public class FPLA004 extends gscm.fwk.base.ProcessUnit{
	public FPLA004(){
		super();
	}
	...
	*/
	public IDataSet fPLA004QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA004 fu = (DPLA004) lookupFunctionUnit(DPLA004.class);
			...
			rdPivot = du.dPLA00402(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00405(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00404(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00403(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA004QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA004 du = (DPLA004) lookupDataUnit(DPLA004.class);
			IRecordSet rs = du.dPLA00401(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA004QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA004 du = (DPLA004) lookupDataUnit(DPLA004.class);
			IRecordSet rs = du.dPLA00406(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
