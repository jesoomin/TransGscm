...
*/
public class FPLA026 extends gscm.fwk.base.ProcessUnit{
	public FPLA026(){
		super();
	}
	...
	*/
	public IDataSet fPLA026QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA026 fu = (DPLA026) lookupFunctionUnit(DPLA026.class);
			...
			rdPivot = du.dPLA02602(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02605(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02604(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02603(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA026QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA026 du = (DPLA026) lookupDataUnit(DPLA026.class);
			IRecordSet rs = du.dPLA02601(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA026QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA026 du = (DPLA026) lookupDataUnit(DPLA026.class);
			IRecordSet rs = du.dPLA02606(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
