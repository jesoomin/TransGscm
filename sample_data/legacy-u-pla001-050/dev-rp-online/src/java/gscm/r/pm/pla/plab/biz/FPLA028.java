...
*/
public class FPLA028 extends gscm.fwk.base.ProcessUnit{
	public FPLA028(){
		super();
	}
	...
	*/
	public IDataSet fPLA028QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA028 fu = (DPLA028) lookupFunctionUnit(DPLA028.class);
			...
			rdPivot = du.dPLA02802(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA02805(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA02804(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA02803(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA028QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA028 du = (DPLA028) lookupDataUnit(DPLA028.class);
			IRecordSet rs = du.dPLA02801(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA028QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA028 du = (DPLA028) lookupDataUnit(DPLA028.class);
			IRecordSet rs = du.dPLA02806(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
