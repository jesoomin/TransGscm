...
*/
public class FPLA006 extends gscm.fwk.base.ProcessUnit{
	public FPLA006(){
		super();
	}
	...
	*/
	public IDataSet fPLA006QrySelectMainList(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		
		try{
			DPLA006 fu = (DPLA006) lookupFunctionUnit(DPLA006.class);
			...
			rdPivot = du.dPLA00602(requestData, onlineCtx).getRecordSet("PIVOT_LIST");
			...
			IRecordSet sDateTimeMap = du.dPLA00605(requestData, onlineCtx).getRecordSet("DATETIME_MAP");
			
			if(strChkSubTotal.equals("Y")) rs = du.dPLA00604(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			else						   rs = du.dPLA00603(requestData, onlineCtx).getRecordSet("MAIN_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA006QrySelectRev(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA006 du = (DPLA006) lookupDataUnit(DPLA006.class);
			IRecordSet rs = du.dPLA00601(requestData, onlineCtx).getRecordSet("REV_LIST");
	 ...
	 * @return 처리결과 DataSet 객체
	*/
	public IDataSet fPLA006QrySelectRevPeriod(IDataSet requestData, IOnlineContext onlineCtx){
		IDataSet responseData = new DataSet();
		try{
			DPLA006 du = (DPLA006) lookupDataUnit(DPLA006.class);
			IRecordSet rs = du.dPLA00606(requestData, onlineCtx).getRecordSet("MAIN_LIST");
			responseData.putRecordset("MAIN_LIST", rs);
		} catch (BizRuntimeException be){
			throw be;
		} catch (Exception e){
